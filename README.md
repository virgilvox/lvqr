...
|---|---|---|
| `lvqr-core` (Rust) | `cargo add lvqr-core` | Shared types, `EventBus`, admin client. Latest: 1.1.0 (crates.io). |
| `@lvqr/core` (TS) | `npm i @lvqr/core` | MoQ-Lite subscriber over WebTransport, WebSocket fMP4 fallback, full admin client: config reload, stream-key CRUD, `serverInfo`, `streamDetail`, `transcodeLadders` + `addRendition` / `removeRendition`, `agents` + `addAgent` / `removeAgent`, `archive`, `ingestListeners` + `stopIngestListener`, `broadcasts` + `stopBroadcast` (truly disconnects a live publisher across RTMP / WHIP / SRT / RTSP), `logsStreamUrl` (SSE), plus health / stats / mesh / SLO / wasm-filter; `MeshPeer` WebRTC DataChannel relay with `pushFrame` + `onChildOpen` + `parentPeerId` + `forwardedFrameCount` + `MeshConfig.capacity`. Latest: 1.1.0. |
| `@lvqr/player` | `npm i @lvqr/player` | Drop-in `<lvqr-player>` web component with MSE fallback. Latest: 1.1.0. |
...
| `@lvqr/admin-ui` | `npm i @lvqr/admin-ui` | Operator admin console -- Vue 3 SPA, every view backed by a live route: dashboard, streams + per-broadcast detail, recordings (archive), DVR scrub, ingest-listener inventory with **runtime listener STOP**, live publisher sessions with **per-broadcast kick** across all four ingest protocols, WASM filters, transcode ladders with **runtime rendition add/remove**, agents with **runtime start/stop**, egress SLO, cluster, mesh, federation, auth (stream-key CRUD), provenance, observability, and a **live SSE log tail**. Multi-relay connection profiles, themable via CSS custom properties, plugin plumbing via `window.__LVQR_ADMIN_PLUGINS__`. Static-deploy behind any host (nginx, Caddy, Digital Ocean App Platform). Latest: 1.1.1 (patch over 1.1.0 fixes a Vue compiler config that was silently breaking the DVR view's `<lvqr-dvr-player>` mount; if you installed 1.1.0 or 1.0.0, upgrade). |
| `lvqr` (Python) | `pip install lvqr` | Admin API client at full v1.1.0 parity with `@lvqr/core`: `stream_detail`, `transcode_ladders` + `add_rendition` / `remove_rendition`, `agents` + `add_agent` / `remove_agent`, `archive`, `ingest_listeners` + `stop_ingest_listener`, `broadcasts` + `stop_broadcast`, `server_info`, `logs_stream_url`, plus the existing v1.0 surface (`config_reload_status` / `trigger_config_reload`, `list_streamkeys` / `mint_streamkey` / `revoke_streamkey` / `rotate_streamkey`, health / stats / mesh / SLO / wasm-filter). `bearer_token` kwarg, `@dataclass` returns. Latest: 1.1.0. |

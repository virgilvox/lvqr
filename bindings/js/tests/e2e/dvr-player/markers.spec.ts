...
test.describe('@lvqr/dvr-player live RTMP publish (closes session 153 deferred)', () => {
  ...
  test('rtmpPush helper publishes; dvr-player LIVE pill flips into is-live state', async ({ page }) => {
    ...
    try {
      ...
      // fulfill threw mid-flight (e.g., the underlying socket closed
      // between fetch resolution and fulfill completion -- the route
      // was marked handled before the error propagated). Mirrors the
      // sibling handler in the marker-injection test below.
      await page.route('**/127.0.0.1:18190/hls/**', async (route: Route) => {
        let resp: Awaited<ReturnType<typeof route.fetch>>;
        try {
          resp = await route.fetch();
        } catch {
          // Network fetch failed entirely -- surface a 502 so hls.js's
          // retry path runs cleanly. Swallow a secondary error in case
          // the route somehow reached the handled state already.
          try {
        ...
        try {
          ...
          // Route already in a handled state (e.g., the test tore down
          // mid-fulfill). The outcome is determined either way; do not
          // double-fulfill.
        }
      });

//! SRT ingest -> LL-HLS HTTP egress end-to-end integration test.
//!
//! Pushes a synthetic MPEG-TS stream containing H.264 video over
//! SRT into `lvqr serve`, then verifies the HLS playlist at
//! `/hls/srt/default/playlist.m3u8` contains at least one
//! `#EXT-X-PART` URI. This proves the full path:
//! SRT socket -> TsDemuxer -> PES -> Fragment -> HlsFragmentBridge
//! -> MultiHlsServer -> axum HTTP.

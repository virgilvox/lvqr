...
mod tests {
    ...
    /// Regression for a fuzz-found OOM DoS: this 104-byte adversarial
    /// init segment drove `mp4-atom`'s box decoder to attempt a
    /// multi-GB `Vec::with_capacity` (a crafted `saiz`/`uncv` count) ->
    /// AddressSanitizer OOM, reached through `Moov::decode`. Both
    /// detectors parse publisher-supplied init bytes, so it was a
    /// publisher-reachable denial of service. Fixed by capping the
    /// count-driven allocations in the vendored mp4-atom
    /// (`vendor/mp4-atom`, `[patch.crates-io]`). The detectors must
    /// return without panicking or over-allocating; reaching the end of
    /// this test is the assertion. (NOTE: verify on Linux/CI -- macOS
    /// overcommit can hide an over-allocation by satisfying it lazily.)
    /// Corpus seed: `fuzz/corpus/detect_codec_strings/crash-69b662e6...`.
    #[test]
    fn detect_codec_strings_no_oom_on_fuzz_crash_69b662e6() {
        const CRASH: &[u8] = &[

...
mod tests {
    ...
    /// Regression for two fuzz-found DoS bugs in `mp4-atom`'s box
    /// decoder, both reached through `Moov::decode` from publisher-
    /// supplied init bytes (publisher-reachable):
    ///
    /// 1. `crash-69b662e6` (104 bytes): a crafted `saiz`/`uncv` count
    ///    drove a multi-GB `Vec::with_capacity` -> AddressSanitizer OOM.
    ///    Fixed by capping count-driven allocations in the vendored
    ///    mp4-atom.
    /// 2. `crash-2e683b11` (237 bytes): a truncated `subs` box made
    ///    `Buf::slice(4)` panic (out-of-bounds) -> process abort. Fixed
    ///    by clamping the vendored `Buf` slice/advance/remaining
    ///    primitives to the available bytes.
    ///
    /// The detectors must return without panicking or over-allocating;
    /// reaching the end of this test is the assertion. (Verify on
    /// Linux/CI -- macOS overcommit can hide an over-allocation by
    /// satisfying it lazily.) Both inputs are fuzz corpus seeds under
    /// `fuzz/corpus/detect_codec_strings/`.
    #[test]
    fn detect_codec_strings_no_panic_or_oom_on_fuzz_crashes() {
        const CRASH_OOM: &[u8] = &[
            0x00, 0x00, 0x00, 0x20, 0x00, 0x00, 0x04, 0x00, 0x00, 0x63, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x1a, 0x1a, 0x76, 0xff, 0x00, 0x1a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x07, 0x00, 0x00, 0x00, 0x00,
            0x3b, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x63, 0x6d, 0x70, 0x64, 0x73, 0x74, 0x73, 0x64, 0x7a, 0xf6,
            0xff, 0x03, 0x01, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0x20, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0xf1, 0x00, 0x00, 0x28, 0x00, 0x0a, 0x1a, 0x1a, 0x00, 0x00, 0x00, 0x1a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x3f, 0x41, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x61, 0x76, 0x30, 0x32, 0x30, 0xff,
        ];
        const CRASH_PANIC: &[u8] = &[
            ,
        ];
        for crash in [CRASH_OOM, CRASH_PANIC] {
            let _ = detect_video_codec_string(crash);
            let _ = detect_audio_codec_string(crash);
        }
    }

//! [`QsvTranscoder`] + [`QsvTranscoderFactory`].
//!
//! Hardware-encoder backend on Linux + Intel Quick Sync (QSV).
...
//! the GStreamer encoder element for `qsvh264enc` (the H.264
//! encoder provided by the `qsv` plugin in `gst-plugins-bad`,
//! driving Intel's Media SDK / oneVPL stack on Quick Sync silicon).
//! HW-only path is intentional: a HW factory that silently falls
//! back to CPU encoding under load defeats the purpose of an
...
//!   ! videoconvert
//!   ! qsvh264enc bitrate=<kbps> gop-size=60 low-latency=true
//!   ! h264parse
...
//! ```
//!
//! plus a working DRI render node (`/dev/dri/renderD128`). On
//! Tiger Lake and newer, oneVPL is preferred; on Skylake-Coffee
//! Lake the legacy MFX path is used. `qsvh264enc` selects between
//! them automatically.
...
/// GStreamer elements the QSV pipeline requires. `qsvh264enc` lives
/// in the `qsv` plugin from `gst-plugins-bad`; the encoder probes
/// the Intel Media SDK / oneVPL runtime + a usable Intel device at
/// element-factory load time.
...
/// Factory that builds [`QsvTranscoder`] instances for the
/// `"0.mp4"` video track. One factory instance per rendition; the
/// CLI's `--transcode-encoder qsv` switch installs three of these
/// (one per default-ladder rung) instead of
/// [`crate::SoftwareTranscoderFactory`].
pub struct QsvTranscoderFactory {
    ...
    pub fn new(rendition: RenditionSpec, output_registry: FragmentBroadcasterRegistry) -> Self {
        let missing_elements = match gst::init() {
            ...
            Err(err) => {
                warn!(
                    ...
                    error = %err,
                    "QsvTranscoderFactory: gst::init() failed",
                );
        ...
        if !missing_elements.is_empty() {
            warn!(
                ...
                "QsvTranscoderFactory: required GStreamer elements absent; factory will opt out of every build()",
            );
        }
        Self {
            rendition,
...
impl TranscoderFactory for QsvTranscoderFactory {
    fn name(&self) -> &str {
        FACTORY_NAME
    }
    ...
    fn build(&self, ctx: &TranscoderContext) -> Option<Box<dyn Transcoder>> {
        ...
        if looks_like_rendition_output(&ctx.broadcast, &self.skip_source_suffixes) {
            debug!(
                ...
                "QsvTranscoderFactory: skipping already-transcoded broadcast",
            );
            return None;
        }
        Some(Box::new(QsvTranscoder::new(
            ctx.rendition.clone(),
            ctx.broadcast.clone(),
...
impl Transcoder for QsvTranscoder {
        ...
        match WorkerHandle::spawn(WorkerSpawnArgs {
            ...
            Err(err) => {
                warn!(
                    ...
                    rendition = %self.rendition.name,
                    error = %err,
                    "QsvTranscoder: failed to spawn worker; transcoder will drop every fragment",
                );
            }
        }
    }
...
impl WorkerHandle {
    fn spawn(args: WorkerSpawnArgs) -> Result<Self, WorkerSpawnError> {
        ...
        let join = thread::Builder::new()
            .name(format!("lvqr-transcode-qsv:{source_broadcast}:{}", rendition.name))
            .spawn(move || {
                run_worker(
                    pipeline,
                    initial_header,
        ...
        match tx.try_send(bytes) {
            ...
            Err(TrySendError::Full(_)) => {
                ...
                warn!(
                    ...
                    "QsvTranscoder: worker channel full; dropping source fragment",
                );
            }
            Err(TrySendError::Disconnected(_)) => {
                debug!(
                    broadcast = %self.source_broadcast,
                    rendition = %self.rendition,
                    "QsvTranscoder: worker already shut down; fragment discarded",
                );
            }
        }
    }
    ...
    fn shutdown(mut self) {
            ...
            && let Err(panic_payload) = join.join()
        {
            warn!(
                ...
                "QsvTranscoder: worker thread panicked during shutdown",
            );
        }
    }
}
            ...
            .new_sample(move |sink| {
                ...
                if is_header {
                    debug!(
                        ...
                        bytes = payload.len(),
                        "QsvTranscoder: output header buffer received; caching as init_segment",
                    );
...
fn run_worker(
    ...
    if let Err(err) = pipeline.set_state(gst::State::Playing) {
        warn!(
            ...
            rendition = %rendition.name,
            error = %err,
            "QsvTranscoder: failed to set pipeline to Playing",
        );
    ...
    if let Err(err) = appsrc.end_of_stream() {
        warn!(
            ...
            rendition = %rendition.name,
            error = %err,
            "QsvTranscoder: end_of_stream signal failed",
        );
    }
    ...
    if let Err(err) = pipeline.set_state(gst::State::Null) {
        warn!(
            ...
            rendition = %rendition.name,
            error = %err,
            "QsvTranscoder: failed to set pipeline to Null",
        );
    }
...
fn push_buffer(appsrc: &gst_app::AppSrc, bytes: Bytes, is_header: bool) -> bool {
    ...
    match appsrc.push_buffer(buffer) {
        ...
        Err(err) => {
            warn!(error = ?err, "QsvTranscoder: appsrc.push_buffer failed");
            false
        }
    }
}
...
fn wait_for_drain(pipeline: &gst::Pipeline, source_broadcast: &str, rendition: &str) {
    ...
    let Some(msg) = bus.timed_pop_filtered(Some(SHUTDOWN_TIMEOUT), &types) else {
        warn!(
            ...
            "QsvTranscoder: pipeline did not drain within timeout; forcing Null",
        );
        return;
    };
    if let gst::MessageView::Error(err) = msg.view() {
        warn!(
            ...
            error = %err.error(),
            debug = ?err.debug(),
            "QsvTranscoder: pipeline reported error during drain",
        );
    }
}
...
mod tests {
    ...
    fn factory_opts_out_of_non_video_tracks_when_available() {
        ...
        let factory = QsvTranscoderFactory::new(RenditionSpec::preset_720p(), registry);
    ...
    fn factory_returns_transcoder_for_video_track_when_available() {
        let registry = FragmentBroadcasterRegistry::new();
        let factory = QsvTranscoderFactory::new(RenditionSpec::preset_480p(), registry);
        if !factory.is_available() {
            eprintln!(
                "skipping: required GStreamer elements missing {:?}",
                factory.missing_elements()
    ...
    #[test]
    fn qsv_transcoder_output_broadcast_name_concatenates_source_and_rendition() {
        let registry = FragmentBroadcasterRegistry::new();
        let transcoder = QsvTranscoder::new(RenditionSpec::preset_240p(), "live/cam1".into(), registry, None);
        assert_eq!(transcoder.output_broadcast_name(), "live/cam1/240p");
    }

    #[test]
    fn factory_name_is_qsv_for_metric_labels() {
        let registry = FragmentBroadcasterRegistry::new();
        let factory = QsvTranscoderFactory::new(RenditionSpec::preset_720p(), registry);
        assert_eq!(factory.name(), "qsv", "metric labels expect a stable factory name");
    }

...
list) and their 5-artifact status as of 2026-04-16 (session 52):
...
| lvqr-whep | yes (`tests/proptest_packetizer.rs`; H.264 RFC 6184 single-NAL + FU-A round-trip + never-panic) | yes (`fuzz/fuzz_targets/parse_offer_sdp.rs`; str0m SdpOffer::from_sdp_string over arbitrary utf8 bytes) | yes (`tests/integration_signaling.rs`; 12 tests driving the WHEP axum router via `tower::ServiceExt::oneshot` with a stub answerer) | yes (`tests/e2e_str0m_loopback.rs`; in-process str0m client + `Str0mAnswerer` loopback over UDP, real ICE + DTLS + SRTP, asserts at least one decoded `Event::MediaData` on the client side after server-side `on_raw_sample` push) | open (cross-implementation against `simple-whep-client` soft-skip) |
| lvqr-whip | yes (`tests/proptest_depack.rs`; structured never-panic over `split_annex_b` + `annex_b_to_avcc`) | yes (`fuzz/fuzz_targets/parse_annex_b.rs`; libfuzzer over the same Annex B depack surface, session 31) | yes (`tests/integration_signaling.rs`; WHIP axum router via `tower::ServiceExt::oneshot` with a stub ingest answerer) | yes (`tests/e2e_str0m_loopback{,_hevc,_opus}.rs`; in-process str0m client publisher + `Str0mIngestAnswerer` loopback over UDP for H.264, HEVC, and Opus) | open (cross-implementation against `simple-whip-client` soft-skip; symmetric to whep) |
| lvqr-dash | open (MPD renderer is pure typed input; future proptest for `Mpd::render` never-panic invariants) | open (pure value-type surface; no parser attack surface yet) | yes (`tests/integration_router.rs`; `DashServer::router` + `MultiDashServer::router` driven via `tower::ServiceExt::oneshot` for manifest + init + numbered segment round trips, multi-broadcast dispatch, and 404 negatives) | yes (`../lvqr-cli/tests/rtmp_dash_e2e.rs`; real RTMP publish through `TestServer` with `with_dash()`, then raw HTTP/1.1 loopback reads the MPD + `init-video.m4s` + `seg-video-1.m4s` and asserts ftyp/moof prefixes) | open (DASH-IF conformance tool has no macos brew package; will land alongside the self-hosted runner escalation that hosts Apple HTTP Live Streaming Tools) |
| lvqr-rtsp | yes (`tests/proptest_parsers.rs`; 11 never-panic tests over RTSP request parser, Transport header, interleaved framing, RTP header, H.264/HEVC/AAC depacketizers, FU reassembly sequences, fmtp config parser, + interleaved round-trip) | open (proptest covers all parser attack surfaces; fuzz target would add long-run coverage) | via unit tests (44 tests covering proto/session/server/rtp modules with full handshake flows) | yes (`../lvqr-cli/tests/rtsp_hls_e2e.rs`; TCP connect, ANNOUNCE/SETUP/RECORD handshake, interleaved RTP push, HLS playlist verification) | open (no external RTSP conformance tool available; future ffprobe validation of emitted fMP4) |

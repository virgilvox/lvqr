...
`tracking/AUDIT-2026-04-29.md` "Remaining open" priority list
shrinks to:

- C-2 WHEP PLI / FIR -- large cross-crate.
- I-1 WHIP trickle ICE PATCH -- medium, str0m API surgery.
- I-5b RTMP `onStatus(error)` hard reject -- `rml_rtmp` vendor
  patch (companion to the already-shipped I-5 warn+counter).
...
This session also doubled as a full-repo audit pass (no code
shipped from that work; findings recorded inline here). What that
audit verified against the current tree:
...
- HANDOFF.md is at 10,671 lines / ~896 KB; the session-72 audit
  flagged it for rotation past Tier 3 and the recommendation
  still stands. Carving sessions 1-149 to
  `tracking/archive/HANDOFF-pre-v1.0.md` would shrink the live
  file by ~75 % and resolve the recurring 25k-token read-cap
  pain. Deferred from this session to keep the B-5 commit focused.

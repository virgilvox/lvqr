...
## Project Status: v0.4.0 -- **Tier 3 COMPLETE; Tier 4 COMPLETE (8 of 8 items done: 4.1 + 4.2 + 4.3 + 4.4 + 4.5 + 4.6 + 4.7 + 4.8)**; 917 workspace tests on the default gate (+31 transcode-feature lib + 1 transcode-feature integration + 1 transcode-feature e2e), 29 crates; **N unpushed commits on local `main` after session 110 close; `origin/main` head still `ea3bbae`**

**Last Updated**: 2026-04-21 (session 110 close). Session 110 broadens the Tier 4 item 4.7 latency SLO tracker's egress coverage from two surfaces (HLS + DASH after 109 A) to four (HLS + DASH + WS + WHEP). The fifth surface -- pure MoQ subscribers drinking directly from `OriginProducer` -- stays out of server-side measurement scope per the 110 scoping decision that keeps the MoQ wire byte-identical; their latency is a Tier 5 client-SDK item. Local `main` sits at N commits ahead of `origin/main`; no push event in this block. crates.io is unchanged -- 110 additively extends `RawSampleObserver::on_raw_sample` + `SessionHandle::on_raw_sample` with a trailing `ingest_time_ms: u64` parameter, grows the private `SessionMsg::Video` struct variant in `lvqr-whep::str0m_backend`, adds a `Str0mAnswerer::with_slo_tracker` builder, and adds `lvqr-admin` + `lvqr-core` to `lvqr-whep`'s dep list. No crate version bumps.
...
2. **v1.1 item 110 B: WHEP RTP packetizer egress SLO instrumentation** (part of the same feat commit -- the WS + WHEP wiring landed naturally together since both flow through the 110-wide signature extension on the raw-sample observer).
...
3. **v1.1 item 110 C: tests + doc refresh** (part of the same feat commit).
...
* **Three transport labels on the tracker today: `"hls"`, `"dash"`, `"ws"`, `"whep"` -- four, not three**. Unquoted lowercase matches the existing convention + the `docs/slo.md` threshold decision table row headers. No alert-pack YAML edits were needed because the rule pack's rules are scoped on `(broadcast, transport)` generically.

### Ground truth (session 110 close)

* **Head**: feat commit (pending) + this close-doc commit (pending). Local `main` will be N ahead of `origin/main` (the exact count depends on whether the WS + WHEP wiring + close-doc land as two commits or as one feat + one close-doc); pre-session head on `origin/main`: `ea3bbae`.
...
* **WHEP inline unit test asserts the negative (pre-Connected) path only**. The positive (Connected -> record) path runs in the existing `e2e_str0m_loopback*.rs` tests against real DTLS peers; those tests do not currently assert on tracker counters. A future maintainer extending the e2e tests should add a `server.slo().snapshot()` assertion after the first video frame reaches the client.

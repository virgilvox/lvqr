...
**Verification at `ee48c0c` (current HEAD):** CI (gating Test Linux + Format/Lint), Feature matrix (transcode + whisper), Mesh E2E (runs ALL Playwright projects, so the 25 admin-ui specs execute in CI Linux), SDK tests, MPEG-DASH Conformance, Supply-chain, Test Contract, Tier 4 demos, VideoToolbox HW encode (macos) all `success`. Only pre-existing red is `Test (macOS, informational)` `scte35_rtmp_push_smoke` RTMP-handshake flake -- the wave touched zero RTMP/HLS/ingest Rust and the slice-9b ingest changes shipped a feature-agnostic RTMP-listener-stop integration test that exercises the same RTMP path in CI Linux without issue. Local counts: lvqr-admin lib **84**, lvqr-transcode 28, lvqr-agent 12, lvqr-archive 32, lvqr-observability 16; admin-ui Vitest 107 + Playwright **25/25** (7 view + 17 a11y + 1 responsive). New shared test helper `lvqr_test_utils::http::http_delete` (Slice 7) and dev-dep `@axe-core/playwright`.

**Remaining backlog (one mutation surface left; subscriber-kick remains an architectural defer):**
* **Broadcast STOP** (Slice 6): `FragmentBroadcasterRegistry::remove` cleanly drains egress, but cannot force-disconnect a still-publishing client (no per-connection cancel token); `RtmpMoqBridge` holds a `streams: DashMap<broadcast, ActiveStream>` that is the wiring hook. SURFACE THIS AS A DECISION before building -- ship with honest "clears server state; an active publisher reconnects" semantics, or add a per-session kill channel first.
* **Ingest listener START** (the symmetric counterpart to Slice 9b STOP): re-enabling after stop needs a re-bind + re-spawn path threaded through the composition root, plus a way to recreate the listener task from outside `start()`. Intentionally deferred -- one-way STOP is the v1 scope and the inventory now reflects it.
...
README "Runtime mutation surface" sections and this HANDOFF are current as of `ee48c0c`.

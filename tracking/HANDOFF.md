...
* **Top fixes by ROI (post-audit, prioritized):**
  1. **CHANGELOG v1.1.0 entry** -- 20 min, ships with the next push, fixes a real release-tracking gap that lets the published 1.1.0 release look stale.
  2. **CLAUDE.md publishing-order rewrite** -- 10 min, prevents future contributors from following stale guidance.
  3. **lvqr-srt back into test contract** + **admin-ui README Ingest row** -- 5 min combined, fixes real documentation drift.
  4. **CORS `.allow_credentials(false)` + Whisper resampler upgrade + RTMP SCTE-35 CRC + WHEP/WHIP bounded channels + HLS playlist cache** -- bundle into a 1.1.2 patch wave. Combined effort ~1 day.
  5. **Python SDK v1.1.0 parity** -- half-day. Ships as `lvqr 1.1.0` on PyPI. Material gap; embedders relying on the Python client are stuck.
  6. **TLS cert/key flag wiring** -- 4 hours OR removal-with-doc. Operators currently get silently-ignored flags.
  7. **SPS-rotation discontinuity, codec-swap stickiness on HLS master, Whisper polyphase resampler, Opus->AAC HLS transcode rendition** -- v1.2 milestone features.
...
README "Runtime mutation surface" sections + Tier 5 ecosystem table and this HANDOFF are current as of `62f96e4`. The operator console is feature-complete against the kickoff prompt's definition of done, and the v1.1.0 release is live on crates.io + npm + GitHub tag.

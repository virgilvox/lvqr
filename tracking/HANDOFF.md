...
**Verification at `d9cb18d` (current HEAD):** CI (gating Test Linux + Format/Lint), Feature matrix (transcode + whisper), Mesh E2E (runs ALL Playwright projects, so the 25 admin-ui specs execute in CI Linux), SDK tests, MPEG-DASH Conformance, Supply-chain, Test Contract all `success`. The two pre-existing non-gating reds unrelated to this wave remain (`LL-HLS Conformance` ffmpeg cold-start publish 404; `Test (macOS, informational)` `scte35_rtmp_push_smoke` RTMP-handshake) -- they pre-date the wave and the wave touched zero RTMP/HLS/ingest Rust. Local counts: lvqr-admin lib 80, lvqr-transcode 28, lvqr-agent 12, lvqr-archive 32, lvqr-observability 16; admin-ui Vitest 107 + Playwright **25/25** (7 view + 17 a11y + 1 responsive). New shared test helper `lvqr_test_utils::http::http_delete` (Slice 7) and dev-dep `@axe-core/playwright`.

**Remaining backlog (the two runtime-mutation surfaces still need composition-root abort/cancel plumbing -- everything else is done):**
* **Ingest listener STOP** (Slice 9 mutation half): abort a bound ingest listener (RTMP/SRT/RTSP/WHIP) at runtime. Existing per-listener `CancellationToken`s are CLONES of the shared global `shutdown` (cancelling one cancels all), so the refactor is to use `shutdown.child_token()` per listener, register the child tokens in an `Arc<DashMap<String, CancellationToken>>` threaded to AdminState (a new `with_ingest_listeners` snapshot closure + a `with_ingest_stop` mutation closure), and add `GET`/`DELETE /api/v1/ingest/listeners[/{protocol}]`. STOP only is the v1 scope -- "start" (re-enable after stop) needs re-bind + re-spawn, harder; document as one-way disable until restart. The read-only inventory currently ships via server-info bound addresses; the new endpoint becomes the richer source of truth (also reflects runtime-disabled state).
...
README "known v1.0 limitations" + "Runtime mutation surface" sections and this HANDOFF are current as of `d9cb18d`.

...
## Console feature-completeness wave (2026-05-20 -> 2026-05-22): admin-ui made real + runtime CRUD

Multi-session push to make the `@lvqr/admin-ui` operator console genuinely feature-complete: close every placeholder view against a real `/api/v1/*` endpoint, flesh out the partial StreamDetail, then (per an explicit operator decision after a feasibility audit) add deep runtime CRUD for transcode + agents. All work on `main`, committed per slice authored as the human dev (no Claude attribution), each verified before the next. Companion design notes live in the auto-memory `project-console-buildout`. Pattern for every slice: closure-injected endpoint in `lvqr-admin` (kept dependency-light) -> typed `@lvqr/core` client method -> Pinia store -> Vue view -> unit + SDK + integration tests. Feature-gated slices (transcode/whisper) are unverifiable on the GStreamer/whisper-less dev host, so they are PUSH-then-verify via the CI Feature-matrix lanes; non-gated slices verify locally.

**Phase 1 -- read-only foundation (every placeholder view now real):**
* **Slice 1 (`a04fbff`)** `GET /api/v1/streams/{name}` -> per-broadcast `StreamDetailInfo` (tracks w/ codec/kind/timescale/fragments/subscribers/lag) by introspecting `FragmentBroadcasterRegistry`; `AdminState::with_stream_detail`; SDK `streamDetail()` (null on 404); StreamDetail.vue tracks table; `track_kind` helper.
* **Slice 2 (`931293b` + Clone fix `cf6116c`)** `GET /api/v1/transcode/ladders` -> `TranscodeState`; `with_transcode`; SDK `transcodeLadders()`; Transcode.vue. The Clone fix is the canonical PUSH-to-verify lesson: a feature-gated closure cloned `Vec<RenditionInfo>` but the DTO lacked `Clone` -> E0599 only under `--features transcode` (invisible locally; caught by CI Feature matrix).
* **Slice 3 (`05c3de4`)** `GET /api/v1/agents` -> `AgentState`; `with_agents`; SDK `agents()`; Agents.vue. whisper-gated CLI closure.
* **Slice 4 (`1ad7a41`)** `GET /api/v1/archive` -> `ArchiveState` via new `RedbSegmentIndex::list_summaries()` (single scan; note redb ReadOnlyTable has no `iter()` here -> use `range::<&[u8]>(..)`); `with_archive` (NOT feature-gated -> fully local); SDK `archive()`; Recordings.vue deep-links into `/dvr?broadcast=` (Dvr.vue preselects from the query).
* **Slice 5 (`18c7c88`)** `GET /api/v1/logs` SSE live-tail. New `lvqr-observability` `CaptureLayer` + process-global `LogBroadcaster` (bounded ring + tokio broadcast) composed into `init()` under the same EnvFilter; `log_broadcaster()` global. Route lives OFF the header auth middleware (EventSource cannot set Authorization) and self-authenticates a `?token=` query param (documented proxy-log caveat); SDK `logsStreamUrl()`; Logs.vue EventSource viewer (level chips, pause, autoscroll, 1000-line cap). `lvqr-admin` gained a `lvqr-observability` (Tier 0) + `futures` dep.

**Phase 2 -- deep runtime CRUD (operator chose this over read-only-only after I surfaced the cost/risk):** feasibility audit found transcode/agent CRUD = multi-session core-pipeline refactors (factory sets frozen in the `on_entry_created` closure), subscriber-kick infeasible (anonymous egress readers), broadcast-stop feasible-but-can't-force-disconnect-a-live-publisher (no per-connection cancel token). Operator picked "commit to deep transcode/agent CRUD".
* **Slice 7 transcode CRUD (`c2d30d1` + `ade9adb` + `9c42e5f`)** Refactored `TranscodeRunner`: frozen-in-closure factory `Vec` -> shared `RunnerInner{registry clone, RwLock<factories>, DashMap<StatsKey,JoinHandle> tasks, DashMap stats}`. Handle gains `add_rendition`/`remove_rendition`/`renditions`; `add_rendition` retroactively spawns drain tasks vs all live sources and `spawn_for` is idempotent on `StatsKey` so it can't race the callback into a duplicate output; dup-guard keys on `(factory-name, rendition)` so a rendition's `software` + `audio-passthrough` factories both register; a recursion guard skips broadcasts whose final path segment is a LIVE rendition name (outputs are `<source>/<rendition>`). `POST`/`DELETE /api/v1/transcode/ladders` (header-auth-gated) + `transcode_mutate` closure; the introspection closure switched to LIVE `handle.renditions()`. Generic runner is not gstreamer-gated -> 28 local lib tests. CONSTRAINT: needs a startup ladder (runner only installs when `config.transcode_renditions` non-empty); runtime-added renditions aren't advertised in the HLS master playlist (published at startup).
* **Slice 8 agents CRUD (`c019007`)** Mirrored the refactor on `lvqr-agent` (no recursion guard needed -- whisper reads `1.mp4`, writes a `captions` track). Handle gains `add_agent`/`remove_agent`/`agent_names` (dup-guard on factory name). The agent runner now ALWAYS installs under the whisper feature (empty if no startup model) so captions can be started FROM ZERO at runtime. `POST`/`DELETE /api/v1/agents` + `agent_mutate` closure; introspection reads live `agent_names()` + a shared model/window metadata map; SDK `addAgent`/`removeAgent`; Agents.vue start-form + per-agent Stop. Generic runner not whisper-gated -> 12 local lib tests.

**Phase 3 -- lock-in + ingest introspection (post-CRUD):**
* **Harness (`a09ad03`)** First admin-ui Playwright project (none existed -- only `mesh` + `dvr-player`). New `admin-ui` project + a `vite preview` webServer that serves the built SPA; the spec seeds a localStorage connection profile + route-mocks every `/api/v1/*` endpoint, so view-render tests need no live relay. 7 specs: dashboard mount + StreamDetail / Transcode / Agents / Recordings / Ingest / Logs render from mocked data. Preview binds `--host 127.0.0.1` because vite defaults to `localhost` (= ::1 on macOS) which the IPv4 url poller never reaches.
* **README audit (`9ce3333`)** Line-by-line check of `bindings/js/packages/admin-ui/README.md` vs the code: accurate (view->endpoint table, scripts, hash routing, app-config keys, tokens, plugin contract). The `MIT OR Apache-2.0` license is CORRECT -- all four npm SDK packages declare it; AGPL applies to the Rust relay only. Three precision fixes (relay-URL precedence, profile `id` + per-protocol port overrides, plugin script-src).
* **Slice 9 ingest introspection, read-only (`369be60`)** `GET /api/v1/server-info` (bound listener addrs + runtime features, wired session 164) was exposed but NEVER consumed by SDK/UI. Added `@lvqr/core` `serverInfo()` + `ServerInfo` / `BoundAddresses` / `RuntimeFeatures` types; a `serverInfo` Pinia store; Ingest.vue gained an "Ingest listeners" inventory card (RTMP / SRT / RTSP / WHIP -> bound address + listening/disabled). No new relay endpoint -> fully local. Also added a "Quick start" section near the top of the README.

**Post-1.1.0 deep four-axis audit (security / lifecycle / codec / docs+CI).** Wide-surface multi-agent sweep across the 29-crate workspace + bindings/{js,python} + CI workflows + scripts. Filtered through validation pass (some agent findings overstated; flagged below). Ordered by severity AND honest-bug-vs-defensive-improvement:

* **HIGH-confidence shipped bugs / gaps:**
  * **Python SDK is 14 admin methods + 19 types behind the v1.1.0 surface** (`bindings/python/python/lvqr/`). Missing `broadcasts` / `stop_broadcast`, `ingest_listeners` / `stop_ingest_listener`, `transcode_ladders` + `add_rendition` / `remove_rendition`, `agents` + `add_agent` / `remove_agent`, `archive`, `stream_detail`, `server_info`, `logs_stream_url`. The README's Tier-5 ecosystem row claims "Admin API client" parity but it's stuck at v1.0.0's subset. Fix scope: ~half-day to mirror the TypeScript surface.
  * **CHANGELOG.md has no v1.1.0 entry.** v1.0.0 documented (2026-04-28); the entire console wave (broadcast STOP × 4 protocols, ingest STOP, runtime transcode/agent CRUD, SSE log tail, stream detail, archive, server-info, admin-ui consoles) is undocumented in the changelog despite shipping to crates.io + npm. Fix scope: 20 minutes.
  * **SPS / parameter-set rotation mid-stream is silently dropped** (`crates/lvqr-ingest/src/bridge.rs:265-315`). RTMP publisher reissues sequence header with different SPS → `stream.video_config` is overwritten without discontinuity / new init emission. Downstream decoders cache the old SPS + misdecode subsequent IDRs. No test covers it. Fix scope: 1-2 hours (compare SPS bytes; if changed, fire `mark_discontinuity_pending` + emit new init).
  * **Same-broadcast codec-swap stickiness** (already documented in the earlier audit block but worth repeating with the file cite): the HLS bridge's `init.mp4` codec string caches first-publisher's codec. When the WHIP publisher (Opus) disconnects and an RTMP publisher (AAC) attaches, the master playlist still advertises Opus and hls.js can't recover without a full reload. Per `crates/lvqr-hls/src/server.rs:177-196` the discontinuity marker fires on `push_init` but the master codec string doesn't rotate -- player-level `MediaSource.addSourceBuffer` was created for the wrong codec and can never recover mid-session. Fix is non-trivial; v1.2 candidate.

* **MEDIUM-confidence defensive improvements (real but not exploitable today):**
  * **WHEP + WHIP per-session `mpsc::unbounded_channel`s**, three per session: `sample_tx`, `opus_tx`, `trickle_tx` (`crates/lvqr-whep/src/str0m_backend.rs:233-240` + `crates/lvqr-whip/src/str0m_backend.rs:191,233,240`). Producer is the per-frame ingest path; consumer is the per-session str0m poll loop. In practice the str0m output pacing rate-limits producer-side, so unbounded growth isn't catastrophic on a healthy session -- but a single misbehaving session under sustained network stall is unbounded by design. Bound at 512-1024 with drop-oldest + counters. Fix scope: 2 hours.
  * **CORS layer doesn't explicitly disable credentials** (`crates/lvqr-cli/src/lib.rs:109-117`). `tower-http` default with `allow_origin(Any)` is to NOT allow credentials, so it's safe today; the concern is purely "if someone later adds `allow_credentials(true)` without rethinking origin policy, the wildcard becomes exploitable." Add explicit `.allow_credentials(false)` for documentation + future-proofing. Fix scope: 1-line edit.
  * **TLS cert/key CLI flags are parsed but never consumed** (`crates/lvqr-cli/src/config.rs:171-175`). Comments say "Reserved; not consumed yet." Operators expecting `--tls-cert` / `--tls-key` to actually wire a CA-signed cert into the MoQ QUIC surface get the self-signed default silently. Either implement (wire into `RelayServer::init_server`) or remove from CLI + document the limitation. Fix scope: 2-4 hours to implement.
  * **Webhook auth provider doesn't reject localhost / private IPs** (`crates/lvqr-auth/src/webhook_provider.rs:196-221`). SSRF risk only materializes if an attacker compromises the operator's config (e.g. malicious `--config` file) and points `--webhook-auth-url` at AWS metadata. Defense-in-depth: reject `localhost` / `127.0.0.1` / `::1` / RFC1918 ranges at config validation. Fix scope: 1 hour.
  * **Timestamp wraparound on 32-bit RTMP clock** (`crates/lvqr-ingest/src/bridge.rs:330`). `timestamp.saturating_sub(prev)` returns 0 instead of the correct wrapped delta when prev is near `u32::MAX` and now is small. Practical exposure: long-running broadcasts (~49 days). Fix: `wrapping_sub` with `.max(1).min(1000)` clamp. Fix scope: 30 minutes + test.
  * **SCTE-35 passthrough in RTMP lacks CRC validation** (`crates/lvqr-ingest/src/rtmp.rs:682-716`). SRT path does run `parse_splice_info_section` (CRC-check + length validation); RTMP path doesn't. A malformed cuepoint base64 payload could ship onto the `scte35` track verbatim. Not memory-unsafe (Rust), but downstream consumers reading uninitialised fields is a real correctness hazard. Fix: add the same `parse_splice_info_section` validation in `parse_oncuepoint_scte35`. Fix scope: 30 minutes.
  * **Whisper audio resampler is nearest-neighbor** (`crates/lvqr-agent-whisper/src/decode.rs:110-128`). 48kHz / 96kHz inputs aliased into 16kHz with no anti-alias filter. Transcription accuracy degrades on non-16kHz publishers (the common case for real publishers). Fix: switch to `rubato` polyphase resampler. Fix scope: 2-3 hours.
  * **HLS playlist body regenerated per GET** (`crates/lvqr-hls/src/server.rs:540-570`). Manifest rendered to String on every `/playlist.m3u8` fetch. For high-RPS subscribe (hls.js polls every part-target), this is real CPU. Cache rendered body keyed on manifest-version; invalidate on `push_part`/`close_segment`/`push_date_range`. Fix scope: 2-3 hours.
  * **HLS/DASH finalize tasks use `shutdown.clone()` not `shutdown.child_token()`** (`crates/lvqr-cli/src/lib.rs:1156-1209`). Inconsistent with slice-9b/6 patterns; latent bug for any future admin route that wants to stop finalize independently. Fix scope: 1-line per task.
  * **WHIP `whip_bridge_keepalive` drop sequencing.** It's dropped AFTER `tokio::join!` completes but before `axum::serve(...)`'s graceful shutdown has drained in-flight handlers. A WHIP `POST /whip/{broadcast}` request landing 1ms before shutdown could observe a dropped `Arc` mid-handler. Add a 100ms sleep before `drop(whip_bridge_keepalive)` OR refactor to await active handlers. Fix scope: 1 hour.
  * **README's Ingest view documentation row is stale** (`bindings/js/packages/admin-ui/README.md:45`). Lists `/api/v1/server-info` + `/api/v1/streams` as the ingest backers; doesn't mention `/api/v1/ingest` + `/api/v1/broadcasts` that landed in slice 9b + 6. Fix scope: 1-line edit.
  * **CLAUDE.md "Publishing Order" lists 8 of 29 crates** (`CLAUDE.md:38-45`). Predates lvqr-cluster, lvqr-observability, lvqr-cmaf, lvqr-codec, lvqr-fragment, lvqr-moq, lvqr-hls, lvqr-dash, lvqr-rtsp, lvqr-srt, lvqr-whip, lvqr-whep, lvqr-archive, lvqr-record, lvqr-agent, lvqr-agent-whisper, lvqr-transcode, lvqr-auth, lvqr-conformance. Fix scope: 10 minutes (rewrite the section to reflect the actual workspace).
  * **lvqr-srt is commented out of the test contract** (`scripts/check_test_contract.sh:50`). Comment says "will be enabled as they land" -- SRT is shipped + published at 1.1.0 + has full fuzz coverage. Uncomment it. Fix scope: 1-line edit.

* **LOW / INFO (real but lowest-priority):**
  * AAC HE-AAC publishers are reported as `mp4a.40.2` in DASH MPDs because parameter-set extraction reads only `audio_object_type`, not the SBR/PS signature. (`crates/lvqr-cmaf/src/init.rs:849` -- minor display issue).
  * Enhanced-RTMP HEVC/AV1 publishers silently drop (documented as deferred per existing comments; not a regression).
  * Fragment ingest timestamps use `SystemTime::now()` per-fragment without per-burst batching; multi-publisher SLO sampling could see identical wall-clock ms.
  * No dedicated CI test job for lvqr-core / lvqr-moq / lvqr-fragment / lvqr-codec / lvqr-cmaf / lvqr-relay / lvqr-auth / lvqr-cluster / lvqr-observability / lvqr-signal. They're covered transitively by `cargo test --workspace` in the test-Linux lane, which is fine in practice but a feature-interaction bug isolated to one of these crates would only surface if the workspace happens to compile it. No dedicated `-p <crate>` job.
  * No Windows lane (Linux + macOS only).
  * No `--features full` combo lane (each opt-in feature tested individually in feature-matrix; the cross-product isn't).
  * `bindings/js/packages/admin-ui/package.json` lacks an `engines` field; users on Node <18 might hit obscure breakage. Fix: add `"engines": { "node": ">=18.0.0" }`.
  * Query param extraction in the live-playback auth middleware doesn't URL-decode (`crates/lvqr-cli/src/auth_middleware.rs:147-159`). Bearer tokens with `+` / `=` would mismatch. JWTs typically don't contain these characters but the helper should be correct.

* **NOT-A-BUG (validated clean during audit):**
  * JWT `alg=none` rejection (`jsonwebtoken` rejects by default; LVQR delegation is correct).
  * JWKS TTL has a 10s floor (configurable, sensible).
  * Path traversal via broadcast names: the MoQ relay validates names strictly before they reach disk paths (no `..` / control chars / backslash / >255 chars).
  * Fragment registry's `get_or_create` uses double-checked locking; multi-publisher race on same broadcast name doesn't overwrite the Arc.
  * SessionRegistrar pattern is consistently wired across all 4 ingest crates with `DeregisterGuard` (RTMP) / explicit deregister-after-await (SRT/WHIP) / `registered_broadcasts` walk on connection exit (RTSP).
  * Transcode + agent runner lock ordering (`RwLock<factories>` released before `DashMap<tasks>` acquired) -- no deadlock vector.
  * Fragment registry callbacks fire OUTSIDE the registry's RwLock (`crates/lvqr-fragment/src/registry.rs:200-226`) so callbacks can recursively call into the registry without deadlock.
  * Hot-path transcoder calls (`on_start` / `on_fragment` / `on_stop`) wrapped in `catch_unwind` with per-stat panic counters; no adversary panic vector.
  * Slice 9b `ingest_listeners` DashMap entries STAYING after STOP is INTENTIONAL (the row reflects `enabled: false` so the UI can show "stopped" rather than letting the listener silently vanish). The Explore agent flagged this as a leak; it's not.

* **Top fixes by ROI (post-audit, prioritized; status as of `5d32023`):**
  1. ~~**CHANGELOG v1.1.0 entry**~~ DONE at `43fa9e3`. CHANGELOG.md now documents the full console wave.
  2. ~~**CLAUDE.md publishing-order rewrite**~~ DONE at `43fa9e3`. Now lists all 26 publishable crates topologically + 3 internal-only.
  3. ~~**admin-ui README Ingest row**~~ DONE at `43fa9e3`. lvqr-srt test-contract item deferred (audit's claim about srt's coverage was wrong -- it lacks `tests/` and `fuzz/` directories, so uncommenting it in the contract would generate the educational warnings the existing comment is keeping silent; correct fix is to land the missing artifacts first, then uncomment).
  4. ~~**Python SDK v1.1.0 parity**~~ DONE at `5d32023`. `lvqr 1.1.0` on PyPI; 23 new types + 14 new methods + 19 new tests; full parity with `@lvqr/core` 1.1.0. The biggest single user-impact item in the audit, closed.
  5. **CORS `.allow_credentials(false)` + Whisper resampler upgrade + RTMP SCTE-35 CRC + WHEP/WHIP bounded channels + HLS playlist cache + HLS/DASH finalize child_token swap + RTMP 32-bit timestamp wraparound** -- OPEN. Bundle as the next 1.1.2 wave. Combined ~1 day. Force-multiplied by being paired with at least one of the material fixes (#6/#7) so the workspace republish has a real reason.
  6. **TLS cert/key flag wiring** -- OPEN. 4 hours to implement OR removal-with-doc. Operators currently get silently-ignored flags.
  7. **SPS-rotation discontinuity, codec-swap stickiness on HLS master, Whisper polyphase resampler, Opus->AAC HLS transcode rendition** -- OPEN. v1.2 milestone features. Each is independently shippable; the Opus->AAC transcode is the user-facing pain point (WHIP browser publish -> hls.js playback works in Safari only).

**Post-1.1.0 audit follow-through SHIPPED (`5d32023` HEAD).** Three follow-ups executed against the deep four-axis audit's prioritized list:

1. **DVR Vue-compiler bug fixed + admin-ui 1.1.0 -> 1.1.1 published (`f620552` + `e20a5fe`).** End-user walkthrough surfaced that `<lvqr-dvr-player>` silently failed to mount in admin-ui 1.0.0 / 1.1.0 because `vite.config.ts` lacked `compilerOptions.isCustomElement`. Vue treated the kebab-case tag as a missing Vue component and skipped rendering; the route's existing Playwright test was a11y-only and never asserted the element mounted. Fix: added `isCustomElement: tag => tag.startsWith('lvqr-')` to the Vue plugin (covers `<lvqr-dvr-player>` today + any future `<lvqr-*>` element). Locked in a new Playwright functional-mount test (`bindings/js/tests/e2e/admin-ui/views.spec.ts`) that deep-links via `?broadcast=` and asserts `lvqr-dvr-player` mounts + is visible. admin-ui Playwright 26/26 -> 27/27. Published `@lvqr/admin-ui@1.1.1` on npm.
2. **Audit Track A doc tidy (`43fa9e3`)** -- zero-Rust-touch, no version bump. `CHANGELOG.md` v1.1.0 entry now documents the full console wave (every new admin endpoint, runtime CRUD, slice 9b STOP, slice 6 KICK, SDK + admin-ui surfaces, infrastructure changes, known WHIP-Opus-in-HLS limitation, published-artifact inventory). `CLAUDE.md` Publishing Order rewritten from 8-of-29 crates to the full topologically-sorted 26 publishable crates + 3 internal-only crates. `bindings/js/packages/admin-ui/README.md:45` Ingest view row updated to list `/api/v1/{ingest, broadcasts, server-info, streams}` + the STOP / KICK affordances. All 4 JS packages got `"engines": { "node": ">=18.0.0" }` (audit found admin-ui requires Node 18+ but the constraint wasn't pinned).
3. **Python SDK 1.1.0 parity + PyPI publish (`5d32023`).** The audit's single largest user-impact finding was that the Python SDK was 14 admin methods + 19 types behind the v1.1.0 surface -- every Python embedder driving the v1.1.0 admin features (`broadcasts` / `stop_broadcast`, `ingest_listeners` / `stop_ingest_listener`, transcode + agent CRUD, archive, stream_detail, server_info, logs SSE) was stuck at v1.0.0. Closed: 23 new `@dataclass`es in `types.py` mirroring every v1.1.0 serde struct; 14 new `LvqrClient` methods + 7 new `_from_json` helpers in `client.py`; `__init__.py` exports 19 -> 43 symbols; tests 39 -> 58 passing (mock-driven coverage for every new method incl. `stream_detail` 404 -> `None` and `logs_stream_url` token-in-URL encoding); `pyproject.toml` 1.0.0 -> 1.1.0; new Python `CHANGELOG.md` v1.1.0 section. **Published `lvqr 1.1.0` on PyPI** (verified via the PyPI JSON API). README's Tier 5 ecosystem row updated to reflect full parity with `@lvqr/core` 1.1.0. Pure-additive over 1.0.0; no existing method signature changed.

**Deferred from audit follow-through (Track C):** the trivial Rust patches (CORS `allow_credentials(false)`, HLS/DASH finalize `child_token()` swap, RTMP timestamp wraparound, SCTE-35 CRC validation, WHEP/WHIP bounded channels) are each real but bundling them would force a 26-crate workspace republish from 1.1.0 -> 1.1.2 for defensive-only improvements. Right vehicle: a focused 1.1.2 wave paired with at least one material fix (codec-swap stickiness on HLS master, SPS rotation discontinuity, Whisper polyphase resampler). Until then they stay in the audit block below as the prioritized backlog.

**Post-1.1.0 deep audit + DVR fix (`f620552` + follow-ups):** End-user walk-through of the live admin-ui surfaced the first real bug in the 1.1.0 release. Triggering a deep cross-codebase audit. Findings, grouped by severity:

* **BUG (shipped, fixed at `f620552`).** The admin-ui's `vite.config.ts` lacked `compilerOptions.isCustomElement`, so the Vue compiler treated `<lvqr-dvr-player>` as a Vue component and silently failed to mount the DVR scrubber. Present in admin-ui 1.0.0 and 1.1.0 on npm; fixed locally; needs a 1.1.1 patch publish. The Playwright suite NEVER caught it because the DVR route test at `bindings/js/tests/e2e/admin-ui/views.spec.ts:189` is only an a11y assertion -- it never asserts the custom element mounts. That's the GAP this whole audit traces back to.
* **BUG (shipped, NOT LVQR's fault but operator-facing).** `WHIP-published audio is Opus; hls.js cannot append Opus to MSE buffers; HLS playback of WHIP-sourced broadcasts fails with `bufferAppendError`. The HLS bridge correctly advertises `CODECS="...,opus"` per RFC 8216bis -- the player limitation is in hls.js, not LVQR. Symptom is bad operator UX: the admin-ui's Playback view shows a black box with no diagnostic. Two queued mitigations: (a) admin-ui Playback view detects `audio codec == opus` and surfaces a banner ("native HLS players only -- WHIP audio is Opus; publish via RTMP/SRT/RTSP for hls.js compatibility, or open in Safari"); (b) v1.2 feature: optional Opus->AAC transcode rendition on the HLS egress path (mirror of the existing AAC->Opus path on WHEP). Documented as a v1.2 backlog item, not v1.1.x.
* **GAP (test infrastructure).** The Playwright views.spec.ts route-render tests mock the entire `/api/v1/*` surface and assert page text presence -- they don't actually exercise side-effect imports like `@lvqr/dvr-player`'s `customElements.define`. That's why a broken Vue-compiler config slipped through. Closing this requires at least one functional mount assertion per custom-element-bearing view; today that's the DVR view, but the test should be written generically so adding `<lvqr-player>` or any future `<lvqr-*>` element to a view automatically benefits.
* **GAP (multi-publisher race).** Two publishers can collide on the same broadcast name. The fragment registry's `get_or_create` uses double-checked locking (`crates/lvqr-fragment/src/registry.rs:190-227`) so neither overwrites the other's broadcaster Arc, BUT the HLS bridge's `init.mp4` codec string caches the FIRST init segment's codec and serves later fragments through it -- which is what produced the user's WHIP-then-RTMP `bufferAppendError` even after stopping the WHIP publish. The fix (HLS bridge re-emits init on codec change with `EXT-X-DISCONTINUITY` per RFC 8216bis) is partially in place at `crates/lvqr-hls/src/server.rs:177-196` (discontinuity marker fires on re-init) -- but the master playlist's cached codec string doesn't reflect that, so hls.js's `MediaSource.addSourceBuffer` is created for the wrong codec at session start and can never recover without a full reload. There's no integration test for the codec-swap scenario.
* **IMPROVEMENT (operational).** `--transcode-rendition` doesn't probe GStreamer plugin availability at boot. With the feature compiled in but plugins missing, transcoding fails on first publish, not at startup. A boot-time `gstreamer::Plugin::check()` probe + warn (not panic) would surface the misconfig cleanly.
* **NOT-A-BUG.** Cross-checked every admin endpoint against every admin-ui view -- coverage is complete (every endpoint has at least one consumer; no orphans). SessionRegistrar wiring is consistent across all 4 ingest crates with no callsites bypassing it. Feature flags are correctly gated. DASH codec caching handles re-init correctly (per-segment, no permanent state). The general architectural shape is sound.

**Top fixes by ROI (ordered):**
1. ~~Add a Playwright functional-mount assertion for the DVR view to close the exact gap that let the original bug ship.~~ **DONE -- new test at `bindings/js/tests/e2e/admin-ui/views.spec.ts` deep-links via `?broadcast=` and asserts `lvqr-dvr-player` mounts + is visible. admin-ui Playwright now 27/27. `@lvqr/admin-ui 1.1.1` published to npm.**
2. Add an admin-ui Playback-view banner when the master playlist's audio codec is `opus` AND the operator isn't on Safari, explaining the hls.js limitation + listing the unaffected paths (Safari native HLS, WHEP, MoQ). **1 hour, ships in 1.1.1 too.**
3. Integration test: codec-swap scenario (publish via WHIP, stop, publish via RTMP, verify HLS init segment updates + EXT-X-DISCONTINUITY fires + a new MediaSource session would succeed). **2-3 hours, ships as 1.1.2 or v1.2.**
4. Boot-time GStreamer plugin probe. **1 hour, ships when the transcode feature gets touched next.**
5. Opus->AAC transcode rendition for HLS. **Real feature, v1.2 milestone.**

**v1.1.0 SHIPPED (`62f96e4` workspace bump + tag `v1.1.0`):** the entire console buildout wave is now live on every channel. crates.io 26/26 publishable crates at 1.1.0 (Tier 0 archive/auth/codec/core/moq/observability -> Tier 1 fragment/signal -> Tier 2 cmaf/agent/record/transcode/wasm/relay/cluster/mesh -> Tier 3 hls/ingest/agent-whisper/admin -> Tier 4 whep/whip/rtsp/srt/dash -> Tier 5 cli). npm 4/4 packages at 1.1.0 (@lvqr/core -> @lvqr/dvr-player -> @lvqr/player -> @lvqr/admin-ui). PyPI `lvqr` deliberately left at 1.0.0 -- zero commits to `bindings/python/` since its 1.0.0 publish, so no bump warranted. Annotated git tag `v1.1.0` pushed to `origin`. Semver minor because every API addition this wave is backwards-compatible (broadcast STOP routes + types, ingest STOP routes + types, runtime transcode/agent CRUD + types, archive + agent introspection + types, SSE log tail, stream detail). README "Tier 5 ecosystem" table updated to 1.1.0 with the new SDK methods enumerated.

**Slice 6 -- broadcast STOP across all 4 ingest protocols SHIPPED (`769f8b7` RTMP, `3c620b3` SRT, `5eb7ef7` WHIP+RTSP, CI green on rerun):**
Per-publisher kick: `DELETE /api/v1/broadcasts/{name}` cancels a per-publisher `CancellationToken`; the ingest crate's read loop drops out of its `select!`, the TCP/UDP socket closes, subscribers see end-of-stream. The publisher can reconnect immediately as a new session. Foundation is protocol-agnostic via closure injection (lvqr-cli owns the `Arc<DashMap<String, BroadcastSessionEntry>>`; each ingest crate accepts an optional `(register, deregister)` pair) -- no shared trait dep. Per-protocol shapes:
* **RTMP** (`769f8b7`) -- new per-session `CancellationToken` + `handle_rtmp_session` read loop wrapped in `tokio::select!` + RAII `DeregisterGuard` so every exit path (EOF, `?`-bubbled error, admin cancel) deregisters.
* **SRT** (`3c620b3`) -- existing per-connection token switched from `.clone()` to `shutdown.child_token()` so cancelling one session no longer cascades to the listener; register fires before the spawn (a racing DELETE still finds the row); deregister runs after `handle_connection.await`.
* **WHIP** (`5eb7ef7`) -- full `oneshot::channel<()>` -> `CancellationToken` swap through `Str0mIngestSessionHandle` (including the Drop impl that now calls `.cancel()`). Peer addr omitted from the WHIP signature (UDP source addr only arrives later in the str0m poll loop) -- CLI adapter inserts `peer: None`.
* **RTSP** (`5eb7ef7`) -- per-TCP `conn_shutdown` switched to `shutdown.child_token()`; new `conn_shutdown` field on `ConnectionState` (parent of the existing `conn_cancel` -- registering the grandchild would not work, child cancellation does not propagate up; this was the bug the RTSP integration test caught on the first run); register fires in `handle_announce` once the broadcast name is known; deregister walks `registered_broadcasts` on connection exit so the multi-session-per-TCP case (uncommon, but valid) cleans up every row.

Each protocol got an integration test that publishes a real session, polls until the entry surfaces in `/api/v1/broadcasts`, kills it via DELETE, and -- STRONGEST assertion -- reads from the publisher's transport stream and asserts EOF / disconnect (RTMP + RTSP TCP EOF; SRT socket close within 2s reflecting poll-tick teardown latency). WHIP relies on structural mimicry + existing `whip_hls_e2e` back-compat coverage (writing a WebRTC publisher in-process is heavy). lvqr-admin lib 84 -> 88 (4 new closure-driven unit tests for the foundation); 3 new lvqr-cli integration test binaries.

The CI Test (Linux) lane failed once on `5eb7ef7` with zero test output captured (matches the workflow's own warning about lld memory pressure from too many integration-test binaries linking in parallel under `--jobs 2`); a rerun was green. The flake recurred on `5d8cc78` (a docs-only commit -- proving the failure mode is workspace-wide link pressure, not code-specific) with the literal `collect2: fatal error: ld terminated with signal 7 [Bus error], core dumped` symptom this time. Mitigated at `ffeb362` by consolidating the three `broadcast_stop_*_admin_route.rs` files into one binary -- the three protocol tests stay independent (each gets its own `#[tokio::test]` + `TestServer`), only the link step is cheaper. Workspace integration-test-binary count is back to what it was at `769f8b7`. The header comment on the consolidated file documents the rationale so the next person who sees it understands why three protocols share one binary.

**Slice 9b -- ingest listener STOP mutation SHIPPED (`ee48c0c`):**
Per-listener `shutdown.clone()` at the four ingest sites (rtmp / whip / srt / rtsp) switched to `shutdown.child_token()`, registered in an `Arc<DashMap<String, IngestListenerEntry>>` and surfaced to operators as `GET /api/v1/ingest` (live inventory with each row's `enabled` state) plus `DELETE /api/v1/ingest/{protocol}` (200 stopped, 200 already_stopped idempotent, 404 unknown, 503 unwired). STOP closes the OS socket -- the integration test asserts a new TCP connect to the bound port is REFUSED after the stop, not just bookkeeping flipped. Re-enable requires a relay restart (the row stays in the inventory as `enabled: false` so the console shows "rtmp stopped" rather than the listener silently vanishing). Each ingest `_fut` got a cascade-cancel gate -- when the listener exited because its own child token was cancelled (operator stop OR parent shutdown), the existing `shutdown_on_exit_X.cancel()` is skipped, so an operator stop no longer tears down the whole relay (the bug the integration test caught on the first try). SDK + admin-ui + Playwright extended to mirror; Ingest.vue switched to the live `/api/v1/ingest` endpoint as the source of truth (richer than server-info's static bound block because each row carries live `enabled` state) and grew a per-row Stop button gated on enabled. lvqr-admin lib 80 -> 84, lvqr-cli ingest_admin_route 1/1, admin-ui Playwright 25/25.

**Phase 3 -- polish (docs + a11y + responsive) COMPLETE:**
* **Console quick-start at the top of the root README (`92b9d3b`)** Two-step block (run the relay; run the admin-ui via `npm install && npm run dev`) right under the existing `lvqr serve` snippet, plus a one-paragraph summary of what `@lvqr/admin-ui` is (every view backed by a live route, runtime transcode/agent CRUD, SSE log tail) with a link to the package README. The console is no longer buried at the SDK table.
* **a11y pass + automated axe gate (`8a9d60c`)** Added `@axe-core/playwright` and a per-view a11y assertion (17 routes, no violations; `color-contrast` excluded -- the palette is design-system `tokens.css`, not a code defect). Fixed every violation: `region` (Topbar `<div>` -> `<header>` banner -> all content sits in a landmark: rail = `<aside>`/`<nav>`, `<main>`, `<footer>`), `scrollable-region-focusable` (the scrollable `<main>` gets `tabindex=0` so keyboard users can scroll views whose content has no focusable element, e.g. an empty/disabled state), `heading-order` (`EmptyState` `<h3>` under PageHeader `<h1>` -> `<h2>`), `select-name` (DVR broadcast picker `<select>` aria-label), `empty-table-header` (Transcode ladder action column gets a visually-hidden "Actions" header via a tiny `.vh` utility).
* **Responsive 1023px sweep + mobile-overflow gate (`d9cb18d`)** Every view scrolled horizontally by +49px at a 390px viewport. Root cause was the shell: the topbar's non-shrinkable content (brand + version + connection label + buttons + large gaps) forced the mobile grid's `1fr` track past the viewport (a `1fr` track's minimum is `min-content`). Fix: App.vue mobile grid `1fr` -> `minmax(0, 1fr)` (caps the track at the viewport so no child's min-content can widen it past the screen) + the topbar at mobile drops the version badge + connection label (the avatar stays clickable -> connection drawer) and tightens gap/padding. New permanent harness assertion: no horizontal page overflow at 390px across the 17 routes.

**Verification at `5d32023` (current HEAD; post-audit follow-through on top of the `v1.1.0` tag at `62f96e4`):** all gating CI lanes green on `ffeb362` / `5eb7ef7` / `3c620b3` / `769f8b7` (Mesh E2E, CI Test Linux, Feature matrix, SDK tests, MPEG-DASH Conformance, LL-HLS Conformance, Tier 4 demos, VideoToolbox HW encode, Supply-chain, Test Contract). The `5d32023` commit is Python-SDK + docs only; doesn't touch Rust, JS, or admin-ui builds. Local counts: lvqr-admin lib 88, lvqr-rtsp lib 120, lvqr-whip lib 40, lvqr-srt lib 11, lvqr-ingest lib 11; admin-ui Vitest 107 + Playwright **27/27** (7 view + 17 a11y + 1 responsive + 1 broadcast-Kick + 1 DVR-mount-regression); **Python `lvqr` 39 -> 58** (39 v1.0 + 19 v1.1.0). Published artifacts: crates.io 26 crates at **1.1.0**; npm `@lvqr/{core, dvr-player, player}` at **1.1.0** + `@lvqr/admin-ui` at **1.1.1** (DVR Vue-compiler patch); PyPI `lvqr` at **1.1.0** (full v1.1.0 admin parity). Git tag `v1.1.0` on `62f96e4`.

**Verification at `62f96e4` (the `v1.1.0` tag; release bump on top of `ffeb362`):** Mesh E2E (the 26 admin-ui Playwright specs incl. the live-publisher-sessions Kick assertion + the 17-route a11y gate + the responsive overflow gate), CI Linux (rerun green; full workspace cargo test under `--jobs 2 -- --test-threads=2`), Feature matrix, SDK tests, MPEG-DASH Conformance, LL-HLS Conformance (also green this run, the documented ffmpeg-cold-start flake did not fire), Tier 4 demos, VideoToolbox HW encode, Supply-chain, Test Contract all `success`. The only red is the pre-existing documented `Test (macOS, informational)` `scte35_rtmp_push_smoke` RTMP-handshake flake -- unchanged since the wave began and untouched by any of the Slice 6 work. Local counts: lvqr-admin lib **88**, lvqr-rtsp lib **120**, lvqr-whip lib 40, lvqr-srt lib 11, lvqr-ingest lib 11, lvqr-transcode 28, lvqr-agent 12, lvqr-archive 32, lvqr-observability 16; admin-ui Vitest 107 + Playwright **26/26** (7 view + 17 a11y + 1 responsive + 1 broadcast-Kick). New shared test helper `lvqr_test_utils::http::http_delete` (Slice 7); dev-dep `@axe-core/playwright`; new lvqr-whip dep `tokio-util` (CancellationToken).

**Verification at `ee48c0c`:** CI (gating Test Linux + Format/Lint), Feature matrix (transcode + whisper), Mesh E2E (runs ALL Playwright projects, so the 25 admin-ui specs execute in CI Linux), SDK tests, MPEG-DASH Conformance, Supply-chain, Test Contract, Tier 4 demos, VideoToolbox HW encode (macos) all `success`. Only pre-existing red is `Test (macOS, informational)` `scte35_rtmp_push_smoke` RTMP-handshake flake -- the wave touched zero RTMP/HLS/ingest Rust and the slice-9b ingest changes shipped a feature-agnostic RTMP-listener-stop integration test that exercises the same RTMP path in CI Linux without issue. Local counts: lvqr-admin lib **84**, lvqr-transcode 28, lvqr-agent 12, lvqr-archive 32, lvqr-observability 16; admin-ui Vitest 107 + Playwright **25/25** (7 view + 17 a11y + 1 responsive). New shared test helper `lvqr_test_utils::http::http_delete` (Slice 7) and dev-dep `@axe-core/playwright`.

**Remaining backlog (the console buildout's scoped backlog is COMPLETE; what remains is architectural deferral):**
* **Ingest listener START** (the symmetric counterpart to Slice 9b STOP): re-enabling after stop needs a re-bind + re-spawn path threaded through the composition root. Intentionally deferred -- one-way STOP is the v1 scope and the inventory reflects it (`enabled: false` on a stopped row).
* **Subscriber kick**: infeasible without a new global per-connection egress session registry (HLS/DASH/WHEP/WS/MoQ readers are anonymous). Architectural; defer unless the operator explicitly wants that change.

README "Runtime mutation surface" sections + Tier 5 ecosystem table + CHANGELOG.md + CLAUDE.md publishing-order + Python `bindings/python/CHANGELOG.md` and this HANDOFF are current as of `5d32023`. The operator console is feature-complete against the kickoff prompt's definition of done; the v1.1.0 release is live on crates.io (26 crates) + npm (4 packages, admin-ui at 1.1.1) + PyPI (`lvqr` 1.1.0 with full admin parity) + GitHub tag `v1.1.0`.

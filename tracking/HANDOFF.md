...
## Project Status: v0.4.0 -- **Tier 3 COMPLETE; Tier 4 COMPLETE** (8 of 8 core items; `examples/tier4-demos/` exit criterion tracked in v1.1 checklist). **Phase A v1.1 in-flight: session 111-A (docs sweep) + session 112 (live HLS + DASH auth + cargo audit CI) shipped; session 111-B (mesh server-side prereqs) is next.** 924 workspace tests on the default gate (+7 from session 112), 29 crates. Origin/main head `11a5989` until push lands on top; local main carries the 111-A docs commit + the 112 feat commit.

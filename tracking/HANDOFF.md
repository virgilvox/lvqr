...
**Verification at `369be60` (current HEAD):** CI (gating Test Linux + Format/Lint), Feature matrix (transcode + whisper), Mesh E2E (runs ALL Playwright projects incl. the 7 admin-ui specs), SDK tests, MPEG-DASH Conformance, Supply-chain, Test Contract all `success`. Two NON-gating reds, both documented pre-existing flakes unrelated to this wave (which touched no RTMP / HLS / ingest Rust): `LL-HLS Conformance` (ffmpeg cold-start RTMP publish never lands -> 404 on the HLS master) and `Test (macOS, informational)` (`scte35_rtmp_push_smoke` RTMP-handshake). Local counts: lvqr-admin lib 80, lvqr-transcode 28, lvqr-agent 12, lvqr-archive 32, lvqr-observability 16; admin-ui Vitest 107 + Playwright 7/7. Shared test helper `lvqr_test_utils::http::http_delete` added this wave.

**Remaining backlog (all the runtime-mutation surfaces still need composition-root abort/cancel plumbing -- the read-only + transcode/agent CRUD halves are done):**
* **Ingest listener STOP** (Slice 9 mutation half): abort a bound ingest listener (RTMP/SRT/RTSP/WHIP) at runtime. Listeners are spawned once in `lvqr_cli::start()` and their JoinHandles go into `ServerHandle`; needs a shared `{protocol -> AbortHandle}` registry threaded to an admin `with_ingest_mutate` closure + `POST/DELETE /api/v1/ingest/...`. The read-only inventory already ships (server-info).
...
* **Subscriber kick**: infeasible without a new global per-connection egress session registry (HLS/DASH/WHEP/WS/MoQ readers are anonymous). Architectural; defer.
* **Phase 3 a11y/responsive polish pass**: keyboard nav, focus management on the modals/drawers, aria-labels, 1023px breakpoint sweep across all views; consider adding axe-core to the Playwright harness for automated assertions.

README "known v1.0 limitations" + "Runtime mutation surface" sections and this HANDOFF are current as of `369be60`.

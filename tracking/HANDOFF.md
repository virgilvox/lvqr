...
## Project Status: v0.4.0 -- **Tier 3 COMPLETE; Tier 4 COMPLETE** (8 of 8 core items; `examples/tier4-demos/` exit criterion tracked in v1.1 checklist). **Phase A v1.1: sessions 111-A (docs) + 112 (live HLS + DASH auth + cargo audit CI) + 111-B1 (signal auth + mesh coordinator accessor + wire-format decision in docs) shipped; 111-B2 (WS-relay peer registration) is next.** 930 workspace tests on the default gate, 29 crates. Origin/main head `791152d` (112); local main carries the 111-B1 feat commit ahead.
...
| **111-B2** | WS-relay peer registration. Wire `MeshCoordinator::add_peer` into `ws_relay_session`: generate a server-side peer_id, register at connect time, send peer_id via leading text frame, `remove_peer` on disconnect. Add Sec-WebSocket-Protocol echo to `lvqr-signal` so `/signal` auth can honor `lvqr.bearer.<token>`. Integration test: two WS subscribers, second receives `AssignParent` naming the first. | low | crates/lvqr-cli/src/lib.rs (post-split target), crates/lvqr-signal/src/signaling.rs, crates/lvqr-cli/tests/mesh_ws_registration_e2e.rs |

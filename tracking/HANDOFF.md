...
**Verification at `c019007`:** Feature matrix (transcode + whisper lanes), CI gating (Test Linux + Format/Lint), and SDK tests all `success`. The lone red is `Test (macOS, informational)` = the documented pre-existing `scte35_rtmp_push_smoke` RTMP-handshake flake (continue-on-error, unrelated). Local counts: lvqr-admin lib 80, lvqr-transcode 28, lvqr-agent 12, lvqr-archive 32, lvqr-observability 16; admin-ui Vitest 105. New shared test helper `lvqr_test_utils::http::http_delete`.

**Remaining backlog:** Slice 6 broadcast-stop (feasible-with-caveat: `registry.remove` drains egress but can't force-disconnect a live publisher; `RtmpMoqBridge` has a `streams` DashMap that could be wired); Slice 9 ingest-listener stop (JoinHandle registry + abort); admin-ui Playwright e2e harness (none exists yet -- only `mesh` + `dvr-player` projects; would lock in all 8 view changes); Phase 3 polish/a11y. README "known v1.0 limitations" updated this wave to reflect the now-real + now-mutable views.

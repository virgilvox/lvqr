...
| 119 | Nightly 24h soak CI job (scheduled workflow; soft-fail for first week then hard). Enable whisper, cluster, transcode features in a CI matrix. Split into three legs across three sessions: feature-matrix (session 123, 118-B/119-A), whisper-scheduled (session 125, 119-B), soak-scheduled (session 127, 119-C). All three legs SHIPPED. |
...
| **122-D** | **SHIPPED** in session 132. Fourth slice of the shared-helpers refactor; drains the `http_get` duplication surface across the 8 non-RTMP test files. No new shared module. All 8 files consume `lvqr_test_utils::http::{HttpGetOptions, http_get_with}` (a few also consume the shared `HttpResponse` where the local shape matched it byte-for-byte):

* `auth_integration.rs` -- kept the local `HttpResponse { status, body: String }` struct (the file's call sites assume `body: String`, not `Vec<u8>`); the wrapper runs `String::from_utf8_lossy(&resp.body).into_owned()` inline. Bearer-dispatch + 5 s TIMEOUT preserved. 6 / 6 tests pass.
* `cluster_redirect.rs` (feature-gated on `cluster`) -- replaced the `http_get_raw(addr, path) -> (u16, Option<String>)` tuple-returning helper with a 4-line wrapper that calls `http_get(addr, path)` from the shared module and projects `resp.header("location").map(|v| v.to_string())` out as the Option<String>. Dropped ~40 LOC of raw-TCP + hand-rolled header parse. 3 / 3 tests pass.
* `federation_reconnect.rs` -- same shape as auth_integration (bearer-dispatch + body: String); same thin wrapper approach. 5 s HTTP_TIMEOUT preserved. 1 / 1 test passes.
* `rtsp_hls_e2e.rs` -- standard 2-arg http_get; dropped local `HttpResponse` + parse. Consumes shared `HttpResponse`. 1 / 1 test passes.
* `slo_latency_e2e.rs` -- `http_get(addr, path) -> (u16, Vec<u8>)` tuple-returning wrapper preserved at the callsite-ergonomics layer; the wrapper destructures `resp.status` + `resp.body` from the shared helper. 3 / 3 tests pass.
* `srt_hls_e2e.rs` -- standard 2-arg http_get; dropped local `HttpResponse` + parse. 2 / 2 tests pass.
* `srt_dash_e2e.rs` -- standard 2-arg http_get. 1 / 1 test passes.
* `whip_hls_e2e.rs` -- dropped local `HttpResponse` + `find_header` + the http_get half of `parse_http_response`; http_get now forwards to the shared module; `http_post_sdp` stays local (shared module is GET-only) but returns the shared `HttpResponse` via a narrow local `parse_http_response` kept for that POST path. 1 callsite swap from `find_header(&resp, "location")` to `resp.header("location")`. 1 / 1 test passes.

Total dropped: ~270 LOC of duplicated http_get bodies, HttpResponse structs, and hand-rolled status-line parsers across the 8 files. Total added: ~75 LOC of thin wrappers. With this slice the `http_get` duplication surface in `crates/lvqr-cli/tests/*.rs` is fully drained: every test file now consumes `lvqr_test_utils::http` either directly or through a thin wrapper. `cargo fmt --all --check` + `cargo clippy --workspace --all-targets -- -D warnings` clean on Rust 1.95; default-gate workspace count unchanged at **991/0/3**. RTMP handshake helper remains un-factored; that is PLAN row 122-E territory and stays deferred for a separate session because the per-file variance (`rtmp_client_handshake` vs `rtmp_handshake`, InProgress-write-before-continue vs write-if-non-empty branches) wants a dedicated design pass rather than a mechanical port. | Drains the http_get duplication surface to zero across all test files. Sessions 129+130+131+132 together took the number from 0 to 22 test files consuming the shared modules; the remaining refactor slice is the RTMP handshake helper (PLAN 122-E).

| **122-C** | **SHIPPED** in session 131. Third slice of the shared-helpers refactor. No new shared module; consumes both the session-129 `lvqr_test_utils::http` and the session-130 `lvqr_test_utils::flv` modules. Migrated 9 more test files off their local duplicates: `archive_dvr_read_e2e` (FLV only; http_get already migrated in 129), `playback_signed_url_e2e` (FLV only; http_get already migrated in 129), `wasm_frame_counter` (FLV; no http_get in this file), `wasm_hot_reload` (FLV; no http_get in this file), `rtmp_ws_e2e` (FLV; no http_get -- file uses ws subscribe over the bridge), `captions_hls_e2e` (http_get only; no FLV -- file pushes synthetic Fragments straight onto the FragmentBroadcasterRegistry rather than through RTMP), `transcode_ladder_e2e` (full FLV + audio + http_get with `Result<HttpResponse, String>` wrapper preserved via thin always-Ok wrapper; feature-gated on `transcode + rtmp`), `whisper_cli_e2e` (FLV + audio + #[ignore]'d; feature-gated on `whisper`), `rtmp_whep_audio_e2e` (FLV via `flv_audio_aac_lc_seq_header(3, 2)` parameterized helper for its 48 kHz/stereo case + 1-callsite swap from `flv_video_keyframe(cts, nalu)` to `flv_video_nalu(true, cts, nalu)` + `find_header(&resp, "location")` swapped for `resp.header("location")` exposed by shared `HttpResponse`; feature-gated on `transcode`). Per-file re-runs: archive_dvr_read 4/0/0, playback_signed_url 3/0/0, wasm_frame_counter 1/0/0, wasm_hot_reload 1/0/0, rtmp_ws 1/0/0, captions_hls 2/0/0; transcode_ladder + whisper + rtmp_whep_audio compile-verified via `cargo check --features <gate>` (whisper locally; transcode locally after brew install gstreamer; rtmp_whep_audio locally with same; runtime exercised by `feature-matrix.yml`). Default-gate workspace count unchanged at 991/0/3 (pure code-dedup; the 6 default-gate files together drop ~280 LOC of duplicated http_get + FLV bodies in exchange for ~30 LOC of thin wrappers + module imports). With this slice the FLV duplication surface is fully drained: zero test files in `crates/lvqr-cli/tests/` carry local `flv_video_seq_header` / `flv_video_nalu` / `flv_audio_*` definitions. RTMP handshake helper still un-factored (separate slice; subtle per-file variance). The 7 non-RTMP test files that still carry local `http_get` (`cluster_redirect`, `federation_reconnect`, `rtsp_hls_e2e`, `slo_latency_e2e`, `srt_hls_e2e`, `srt_dash_e2e`, `whip_hls_e2e`, `auth_integration` -- 8 actually, off-by-one in earlier counts) are deferred to a follow-up slice. | Drains the FLV duplication surface to zero across 14 RTMP-class files (sessions 129+130+131 combined); leaves only http_get migration on non-RTMP tests and the RTMP handshake helper for a follow-up slice.

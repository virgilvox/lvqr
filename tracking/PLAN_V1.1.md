...
|---|---|---|
| **111-A** | **SHIPPED.** Docs accuracy sweep. Fixed README drift on published SDKs (now lists `@lvqr/core` / `@lvqr/player` at 0.3.1 in the Client libraries table), WASM mutation capability (the "WASM filter" Programmable data plane bullet now states drop / rewrite), mesh client state (Peer mesh feature bullet documents the shipped `MeshPeer`), Tier 4 exit criterion (Roadmap section names `examples/tier4-demos/demo-01.sh`). Added "Known v0.4.0 limitations" section naming the HLS/DASH auth gap (later flipped to Fixed in 112) + WHEP AAC drop (later flipped to Fixed in 113). `docs/mesh.md` flipped to reflect the JS `MeshPeer` ships today. | The README actively misleads anyone landing on the repo today. Zero risk, immediate value. |
...
| **118** | **SHIPPED** across sessions 122 (118-A) + 123 (118-B/119-A). Original scope: SDK completion -- flesh out `@lvqr/core` admin client to cover all `/api/v1/*` routes (mesh, slo, cluster, federation). Add Vitest smoke tests. Add pytest invocation in CI. |
| **119** | **SHIPPED** across sessions 123 (119-A: feature-matrix workflow) + 125 (119-B: whisper-scheduled) + 127 (119-C: soak-scheduled). Original scope: nightly 24h soak CI job (scheduled workflow; soft-fail for first week then hard). Enable whisper, cluster, transcode features in a CI matrix. Split into three legs across three sessions; all three legs SHIPPED. The 60-minute daily cadence on `soak-scheduled.yml` deviates from the original "24 h" scope because GitHub-hosted runners cap at 360 min per job; documented in the workflow header + README Known Limitations bullet; self-hosted-runner variant for true 24 h is a v1.2 follow-up. |
| **119-C** | **SHIPPED** in session 127. New `.github/workflows/soak-scheduled.yml` runs `lvqr-soak` on a daily cron (07:23 UTC, offset from the existing `whisper-scheduled.yml` 05:23 cron so the two scheduled workflows do not compete for runner slots). Duration is 60 minutes (the CI-pragmatic shape that fits comfortably below GitHub-hosted runners' 360-minute ceiling); the soak driver's own pass/fail logic asserts on RSS + FD drift + minimum RTP/RTCP per subscriber. A true 24h nightly on GH-hosted is infeasible + a self-hosted runner variant is a v1.2 follow-up, explicitly documented in the workflow's header comment. Release build via `cargo build -p lvqr-soak --release` so the soaked binary shape matches what operators deploy (avoids debug-assert overhead skewing CPU numbers). stdout + `tracing` logs tee-captured into `soak-artifacts/soak-report.txt`, uploaded via `actions/upload-artifact@v4` with `name: soak-report-${{ github.run_id }}` + 30-day retention for drift-investigation windows. `continue-on-error: true` initially per the convention every dedicated workflow since `hls-conformance.yml` has followed; promotes to a required check after the first clean run on main. `timeout-minutes: 80` gives comfortable headroom for the 60 min soak + warm-cache build + toolchain setup + artifact upload. `Swatinem/rust-cache@v2` with a namespaced `prefix-key: soak-scheduled-v1` so this workflow's release-profile cache does not collide with the default-gate debug-profile cache. | Closes the last leg of PLAN row 119. Every feature gate + scheduled coverage obligation from the v1.1 plan's "Phase C" scoping line is now discharged. |
| **120** | **SHIPPED** in session 126. New `JwksAuthProvider` in `lvqr-auth` (behind a new `jwks` Cargo feature that pulls `reqwest` + `tokio` + `url` as optional deps). Async `JwksAuthProvider::new(JwksAuthConfig)` performs an initial synchronous JWKS fetch (fail-fast on unreachable / malformed endpoints) + spawns a background `tokio::spawn` refresh task that loops on `config.refresh_interval` and on `Notify` kicks. Keys cached by `kid` in a `RwLock<HashMap>`; `Drop` aborts the refresh task. Sync `check()` calls `jsonwebtoken::decode_header` for `alg` + `kid`, validates `alg` against an allow-list (default `RS256` + `ES256` + `EdDSA`; HS256 rejected to prevent public-key-as-secret downgrade), looks up the `kid` in the cache, calls `decode::<JwtClaims>` with the correct `Algorithm`. Claims dispatch + scope hierarchy + broadcast-binding logic are identical to `JwtAuthProvider` -- JWKS tokens drop into every per-protocol carrier (RTMP stream key, WHIP/RTSP bearer, SRT streamid, WS ingest subprotocol, WS subscribe) without any other code changes. CLI: new `--jwks-url` + `--jwks-refresh-interval-seconds` flags (with `LVQR_JWKS_*` env equivalents), reusing the existing `--jwt-issuer` + `--jwt-audience` flags for claim binding. Mutually exclusive with `--jwt-secret` (startup error if both set). New `jwks` feature on `lvqr-cli` threaded into `full`. Integration test coverage: 9 wiremock-backed tokio tests in `crates/lvqr-auth/src/jwks_provider.rs` using an rcgen-generated Ed25519 keypair (happy path, tampered signature, unknown-kid deny + refresh kick, key-rotation roundtrip, single-key no-kid, scope enforcement, HS256 pre-verify rejection, initial-fetch failure). 5 CLI parse tests in `main.rs::jwks_cli_tests` covering flag presence, mutex, default value, env override. Default-gate workspace test count unchanged at 968 (new tests are feature-gated on `jwks`, which is off by default). README Known Limitations bullet flipped; README Auth+ops-polish checklist item flipped; CLI reference block gains the two new flags; `docs/auth.md` grows a dedicated "JWKS dynamic key discovery" section documenting enablement, accepted algorithms, key-selection rules, JWK shape, operational notes. | Closes the largest remaining v1.1 auth item + the README `[ ] OAuth2 / JWKS dynamic key discovery` checkbox. |
| **119-B** | **SHIPPED** in session 125. New `.github/workflows/whisper-scheduled.yml` promotes `whisper_cli_e2e` from `#[ignore]` to CI via a daily cron + `actions/cache@v4`-backed `ggml-tiny.en.bin` download (78 MB from Hugging Face; retry loop + 70 MB minimum-size sanity check). Closes the session-121 / 123 Known Limitations bullet: "`whisper_cli_e2e` stays #[ignore] pending cached-model scheduled workflow". Companion to session 123's `feature-matrix.yml` whisper cell which only exercised compile + `whisper_basic` unit tests. Also: SDK reconnect/retry docs -- `docs/sdk/javascript.md` 105 -> 307 lines, `docs/sdk/python.md` 62 -> 215 lines. Every section the session-121 audit named as missing (9-method admin reference + type reference + `connectTimeoutMs` / `fetchTimeoutMs` / `bearerToken` docs + canonical jittered-exponential-backoff reconnect recipe + admin-side retry recipe + MeshPeer `pushFrame` / `onChildOpen`). Python docs add a `0.3.1` -> `main` migration section naming the published-vs-HEAD admin-method skew. | Closes the "SDK reconnect/retry docs" carry-over from session 121 + the "scheduled whisper workflow" carry-over from session 123. The distinct nightly-24h-soak leg of PLAN row 119 remains unshipped. |
| **122-F** | **SHIPPED** in session 134. Sixth slice of the shared-helpers refactor. Extends `crates/lvqr-test-utils/src/rtmp.rs` with three new public helpers that every RTMP-class integration test used to reimplement: `pub async fn send_results(stream, results: &[ClientSessionResult])`, `pub async fn send_result(stream, result: &ClientSessionResult)`, and `pub async fn read_until<F>(stream, session, timeout: Duration, predicate: F) where F: Fn(&ClientSessionEvent) -> bool`. Uses `tokio::time::Instant` + `saturating_duration_since` for the deadline arithmetic. Migrated 14 test files onto the shared helpers (13 in `crates/lvqr-cli/tests/` + 1 in `crates/lvqr-ingest/tests/`). The lvqr-ingest file additionally picked up the handshake + FLV migrations deferred from 130-133. `one_token_all_protocols.rs` still keeps its local Result-returning handshake + `write_outbound` for documented single-caller exemptions. Default-gate count unchanged at 991/0/3. With this slice the shared-helpers refactor is COMPLETE: four primitives factored across sessions 129-134 (http_get in 122-A/D, FLV in 122-B/C, handshake in 122-E, send+read_until in 122-F). | Closes the shared-helpers refactor story. `lvqr_test_utils::{http, flv, rtmp}` are now the canonical sources for HTTP GET, FLV tag math, handshake, and RTMP event-loop helpers across every integration test in the repo (save one_token_all_protocols which keeps documented single-caller exemptions). |
| **122-E** | **SHIPPED** in session 133. Fifth slice. New `crates/lvqr-test-utils/src/rtmp.rs` exposes `rtmp_client_handshake(stream) -> Vec<u8>` (panic variant). Added `rml_rtmp = { workspace = true }` + tokio `io-util` feature to `lvqr-test-utils/Cargo.toml`. 13 test files migrated. `one_token_all_protocols.rs` keeps its Result-returning variant (single-caller with documented error-recovery contract). Drive-by: `cargo clippy --fix` applied `collapsible_match` rewrite across 11 files' `read_until` match blocks (cache invalidation surfaced pre-existing lints). Default-gate count unchanged at 991/0/3. | Closes the RTMP-handshake half of the shared-helpers refactor; `read_until` + `send_results` cluster picked up next session in 122-F. |

| **122-D** | **SHIPPED** in session 132. Fourth slice. Drains the `http_get` duplication surface across 8 non-RTMP test files (`auth_integration`, `cluster_redirect`, `federation_reconnect`, `rtsp_hls_e2e`, `slo_latency_e2e`, `srt_hls_e2e`, `srt_dash_e2e`, `whip_hls_e2e`). All 8 files now consume `lvqr_test_utils::http::{HttpGetOptions, http_get_with}` either directly or via a thin wrapper that preserves the file's historical call-site shape (auth_integration / federation_reconnect keep `body: String` via a `String::from_utf8_lossy` wrapper; cluster_redirect collapses its tuple-returning helper to 4 lines via `resp.header("location")`; slo_latency keeps its `(status, body)` tuple shape). ~270 LOC of duplicated http_get bodies + parsers dropped; ~75 LOC of thin wrappers added. Default-gate count unchanged at 991/0/3. | Drains the http_get duplication surface to zero across all test files. Sessions 129+130+131+132 together took the number from 0 to 22 test files consuming the shared modules. RTMP handshake helper remains for a follow-up slice (PLAN row 122-E). |
| **122-C** | **SHIPPED** in session 131. Third slice. Migrated 9 test files off their remaining local FLV + http_get duplicates onto the shared modules: `archive_dvr_read_e2e`, `playback_signed_url_e2e`, `wasm_frame_counter`, `wasm_hot_reload`, `rtmp_ws_e2e`, `captions_hls_e2e` (default-gate, all locally re-verified), plus `transcode_ladder_e2e` (transcode-feature-gated; preserves the `Result<HttpResponse, String>` wrapper shape via always-Ok forwarder), `whisper_cli_e2e` (whisper feature; compile-verified locally), `rtmp_whep_audio_e2e` (transcode feature; uses `flv_audio_aac_lc_seq_header(3, 2)` for its 48 kHz/stereo case + inlines `flv_video_keyframe` to `flv_video_nalu(true, ..)` + swaps `find_header` to `HttpResponse::header()`). FLV duplication surface fully drained: zero local FLV definitions across the 14 RTMP-class test files. ~280 LOC dropped + ~30 LOC of thin wrappers added. Default-gate count unchanged at 991/0/3. | Drains the FLV duplication surface to zero across 14 RTMP-class files (sessions 129+130+131 combined); leaves only http_get migration on non-RTMP tests and the RTMP handshake helper for follow-up slices. |

| **122-B** | **SHIPPED** in session 130. Second slice of the shared-helpers refactor. New `crates/lvqr-test-utils/src/flv.rs` module centralizes the FLV tag builders every RTMP-ingest integration test file had reimplemented: `flv_video_seq_header()` (H.264 High@L3.1 SPS+PPS; byte-identical across 11 pre-migration files); `flv_video_nalu(keyframe, cts, nalu_data)` (signed-24-bit-BE composition time + AVCC payload; byte-identical across the same 11 files); parameterized `flv_audio_aac_lc_seq_header(sample_freq_index, channels)` plus a `flv_audio_aac_lc_seq_header_44k_stereo` convenience wrapper (byte-identical across 3 pre-migration 44.1 kHz files + reproduces session-114's 48 kHz variant via `(3, 2)`); `flv_audio_raw(aac_data)` (byte-identical across 4 files). 7 unit tests in `flv::tests` lock the byte output against the historical sequences including session-114's `0xAF 0x00 0x11 0x90` seq header. Migrated 5 test files to consume both the session-129 `http_get` helper + the new flv module: `rtmp_archive_e2e` (removed ~60 LOC of `HttpResponse` + `http_get` + `http_get_with_auth` + ~15 LOC of flv builders; kept a 6-line `http_get` wrapper + a 6-line `http_get_with_auth` wrapper preserving the 10-second TIMEOUT + forwarding bearer through `HttpGetOptions::bearer`); `rtmp_hls_e2e` (removed ~45 LOC of `HttpResponse` + `http_get` + `parse_http_response` + ~35 LOC of flv_video_* + flv_audio_* builders; 1 call site updated from `flv_audio_seq_header()` to `flv_audio_aac_lc_seq_header_44k_stereo()`); `rtmp_dash_e2e` (removed ~45 LOC of http helpers + ~18 LOC of flv_video builders); `c2pa_verify_e2e` (removed ~35 LOC of http helpers + ~18 LOC of flv_video builders; c2pa feature-gated); `c2pa_cli_flags_e2e` (removed ~35 LOC of http helpers + ~18 LOC of flv_video builders; c2pa feature-gated). Every migrated test file re-verified passing: 2 / 3 / 2 / 1 / 2 respectively; no regressions. Default-gate workspace count 984 -> 991 (+7 from the new flv unit tests; no new integration tests, no integration-test removals). Remaining 10 test files still carry local http_get duplicates (rtmp_ws_e2e, rtmp_whep_audio_e2e, captions_hls_e2e, cluster_redirect, federation_reconnect, rtsp_hls_e2e, slo_latency_e2e, srt_hls_e2e, srt_dash_e2e, transcode_ladder_e2e, whip_hls_e2e, auth_integration, whisper_cli_e2e, wasm_frame_counter, wasm_hot_reload); RTMP handshake helper still un-factored (the briefing's session-129 note flagged subtle per-file variance in the handshake wrapper, which deferring confirmed by a quick read of the three variants). | Doubles the 122-A slice's reach: HTTP + FLV are the two of the three historical duplication surfaces named in session 129; only the RTMP handshake helper remains. |

| **122-A** | **SHIPPED** in session 129. First slice of the shared-helpers refactor the briefing called for. New `crates/lvqr-test-utils/src/http.rs` module centralizes the raw-TCP `GET` helper every integration test file had reimplemented: `HttpResponse { status, headers, body }` with case-insensitive `header()` lookup + `body_text()` lossy-utf8 view; `HttpGetOptions` with `bearer` / `range` / `extra_headers` / `timeout` fields and `with_bearer` / `with_range` constructors; `http_get(addr, path)` / `http_get_with(addr, path, opts)` / `http_get_status(addr, path)` public functions. Raw-TCP kept (no reqwest/hyper dep) so localhost integration tests do not pay a new TLS build-graph cost. Migrated 4 test files to consume the shared helper: `live_signed_url_e2e` (session 128) now calls `http_get_status` directly; `hls_live_auth_e2e` (session 112) folds its bearer-or-no-bearer dispatch into a 10-line local wrapper over `http_get_with` / `http_get_status`; `playback_signed_url_e2e` (session 124) keeps its thin `http_get_with_bearer` wrapper that forwards into `http_get_with`; `archive_dvr_read_e2e` (session 118) keeps its thin `http_get_with_range` wrapper preserving the 10-second timeout its RTMP-publish + archive-flush + playback flow needs. Every migrated test file re-verified passing: 7 / 7 / 3 / 4 respectively, no regressions. Default-gate workspace count unchanged at 984 (the refactor is pure code-dedup; the shared module's own logic replaces ~200 lines of duplicated test-file code with ~150 lines of centralized module + wrappers). Remaining 13 test files (rtmp_archive_e2e, rtmp_hls_e2e, rtmp_dash_e2e, rtmp_whep_audio_e2e, c2pa_verify_e2e, c2pa_cli_flags_e2e, captions_hls_e2e, cluster_redirect, federation_reconnect, srt_hls_e2e, srt_dash_e2e, rtsp_hls_e2e, slo_latency_e2e, transcode_ladder_e2e, whip_hls_e2e, auth_integration) keep their local http_get duplicates; migrating them is follow-up work that can land incrementally without disturbing the shared module API. FLV tag builders + RTMP handshake helpers also remain un-factored; those are their own follow-up (the FLV tag shape is uniform, the handshake wrapper varies slightly between RTMP test files). | First concrete reduction of the 9+ test-file duplication surface. Establishes `lvqr_test_utils::http` as the pattern future tests adopt. |
| **121-B** | **SHIPPED** in session 128. Extends session 124's HMAC signed-URL primitive from `/playback/*` to live `/hls/*` + `/dash/*`. New `crates/lvqr-cli/src/signed_url.rs` module factors out a shared `SignedUrlCheck` enum + `verify_signed_url_generic` + `compute_signature` helper; `archive.rs` refactored to delegate to them so the playback + live paths share one HMAC primitive. New `pub fn sign_live_url(secret, LiveScheme::Hls \| Dash, broadcast, exp_unix) -> String` re-exported from `lvqr_cli`. Broadcast-scoped design: signature input is `"<scheme>:<broadcast>?exp=<exp>"` so one sig + exp pair grants access to the master playlist + every numbered / partial segment under `/hls/<broadcast>/*` (or `/dash/<broadcast>/*`); path-bound signatures are impractical for LL-HLS partials that roll over every 200 ms. Scheme tag baked into the signed input rejects cross-scheme replay (HLS-minted sig fails DASH verify). `LivePlaybackAuthState` grew a `scheme: LiveScheme` + `hmac_secret: Option<Arc<[u8]>>` field; the session-112 `live_playback_auth_middleware` pre-checks signed URL params before the subscribe-token gate. `lib.rs` hoists the existing `hmac_playback_secret` declaration above the combined-router block so both HLS + DASH spawn closures can capture it. One `--hmac-playback-secret` mints URLs across all three route trees -- operators do not manage two secrets. New integration tests at `crates/lvqr-cli/tests/live_signed_url_e2e.rs` (7 `#[tokio::test]`): valid HLS sig + valid DASH sig + tampered sig + expired URL + cross-scheme rejection (HLS sig on DASH) + cross-broadcast rejection + fall-through-to-subscribe-gate when no sig. 8 unit tests in `signed_url::tests` cover the pure primitive (scheme-bound / broadcast-bound / expiry-bound HMACs, round-trip, cross-scheme deny, NotAttempted semantics). Default-gate workspace count 968 -> 984 (+15 new tests + 1 auto-included ignored). Metric labels `lvqr_auth_failures_total{entry="hls_signed_url" \| "dash_signed_url"}` distinct from the existing `entry="playback_signed_url"`. README Next Up #4 updated to name both sessions + README Auth+ops-polish checklist item flipped; `docs/auth.md` grows a new "Live HLS + DASH signed URLs" subsection with the design rationale + operator example. | Closes the session-124 Known Limitation "HMAC gated on `/playback/*` only". Narrow but real operator use case: sharing a one-off live-stream link with a third party who cannot authenticate. |
...
### Phase D: v1.1 marquee (re-projected; original session-range estimates slipped)

The original Phase D session ranges (122-125 / 126-129 / 130-132 / 133+)
were authored on the assumption that Phase C would close inside session
121. In practice sessions 122-128 absorbed the SDK-completion + JWKS
auth + HMAC playback + scheduled-workflow polish that the original
Phase C row scoping under-estimated, then sessions 129-134 ran the
shared-helpers refactor (PLAN rows 122-A through 122-F) the audit
table did not anticipate. Phase D therefore starts at **session 135**;
the scope below is unchanged but the session windows shift.

| Scope | Notes |
|---|---|
| Mesh data-plane completion | Client-side parent/child relay hardening, actual-vs-intended offload reporting, per-peer capacity advertisement, TURN deployment recipe, 3+ browser Playwright E2E. Flip `docs/mesh.md` to "IMPLEMENTED". README "Next up" item 5. |
| One hardware encoder backend | Pick based on deployment target: VideoToolbox for macOS dev, NVENC for Linux prod. Defer the other three to v1.2. README "Next up" item 6. |
| Stream-modifying WASM pipelines v2 with chaining | Documented v1.1 marquee feature. README "Next up" item 7. |
| Webhook auth provider | Smaller phase-C carryover named in README "Auth + ops polish" `[ ]` and "Next up" item 8. Operator demand below the JWKS row that 120 shipped, but the surface is well-scoped: async `check()` posts AuthContext as JSON to a configured URL and reads `{"allow": bool, "reason": str}`; per-decision LRU cache + background refresh task mirror the JwksAuthProvider session-126 pattern. |
| Authoritative DASH-IF container validator | Wire `dashif/conformance` Docker container into `dash-conformance.yml` alongside (or replacing) the current GPAC MP4Box step. REST-API-only container shape; medium risk; carry-over from 117/118. |
| npm + PyPI publish cycle | Bump `@lvqr/core` + `@lvqr/player` to 0.3.2 on npm + `lvqr` to 0.3.2 on PyPI. Carries the session-122 / 123 9/9 admin expansion + session-126 JWKS public API + session-128 `sign_live_url` public API. Needs publish credentials. |
| Tier 5 ecosystem | Helm chart, Kubernetes operator, Terraform module, docs site. |
...
- Hot config reload.
- Stream-key CRUD admin API (named in README Auth+ops-polish).
...
- True 24 h soak on a self-hosted runner (60 min daily on
  GitHub-hosted shipped in 119-C; the 24 h variant is anti-scoped to
  v1.2 because GitHub-hosted runners cap at 360 min/job).

...
| **111-B2** | WS-relay peer registration (second half of 111-B). Wire `MeshCoordinator::add_peer` into `ws_relay_session`: generate a server-side peer_id, register at connect time, send the peer_id + role to the client as a leading text frame, and call `remove_peer` on disconnect. Integration test: two WS subscribers where the second receives `AssignParent` naming the first. Add Sec-WebSocket-Protocol subprotocol echo to `lvqr-signal` so `/signal` auth can also honor `lvqr.bearer.<token>`. | Narrow scope, clear deliverable. |

...
## 4.3 -- C2PA signed media (3 sessions, 91-93)
...
| 93 | B2 | (a) Cert-chain fixture so the happy-path `c2pa_sign` test unignores: options are (i) rcgen with full extension control (explicit AKI/SKI + basic-constraints criticality + validity window), (ii) vendored CA/leaf under `tests/fixtures/c2pa/` with a 2099 `notAfter`, (iii) vendor `c2pa::CertificateTrustPolicy::passthrough()` behind a test-only feature -- note that c2pa 0.80's `verify.verify_trust` setting is `pub(crate)` so this needs either a c2pa upgrade or a light wrapper. (b) Finalize-asset orchestration: add a broadcast-end lifecycle hook to `lvqr_fragment::FragmentBroadcasterRegistry` (load-bearing primitive for 4.4 + 4.5 too; design the API shape before coding), persist init bytes to disk at first-segment-write time (decision: flat `<archive>/<broadcast>/<track>/init.mp4` vs. `metadata.json` sidecar), extend `lvqr_cli::archive::BroadcasterArchiveIndexer::drain` to call `concat_assets` + `sign_asset_bytes` + `write_signed_pair` on drain termination when `C2paConfig` is `Some`. (c) `GET /playback/verify/{broadcast}` admin route in `lvqr-cli` that reads the signed asset + sidecar manifest and returns asserted identity + validation status as JSON via `c2pa::Reader::from_manifest_data_and_stream`. (d) E2E test `crates/lvqr-cli/tests/c2pa_verify_e2e.rs`: RTMP publish, broadcaster disconnect, hit `/playback/verify`, assert manifest parses + creator matches. | `cargo test -p lvqr-archive --features c2pa --test c2pa_sign` has 2 passed 0 ignored; `cargo test -p lvqr-cli --test c2pa_verify_e2e` green. | pending |

...
## 4.5 -- In-process AI agents framework (3 weeks, 4 sessions, 97-100; COMPLETE)
...
| 98 | B | New crate `crates/lvqr-agent-whisper` (workspace member, AGPL-3.0-or-later, edition 2024). `WhisperCaptionsFactory` opts in only for the audio track (`track_id == "1.mp4"`), returns `None` for video / catalog / other tracks. `WhisperCaptionsAgent` plugs into the session-97-A `AgentRunner` as a `Box<dyn Agent>`; no changes to `lvqr-agent`. Heavy deps (`whisper-rs 0.16` + `symphonia 0.6.0-alpha.2` with the `aac` feature) gated behind a default-OFF `whisper` Cargo feature so `cargo test --workspace` stays fast (no whisper.cpp bindgen + cmake on every push). Always-available surface: `TranscribedCaption`, `CaptionStream` (a `tokio::sync::broadcast`-backed fan-out), `WhisperCaptionsFactory`, the agent (which no-ops `on_fragment` without the feature), the `mdat` extractor (strips a `moof + mdat` fragment payload to its raw AAC frame bytes), and the `asc` extractor (parses an `esds` chain in a CMAF audio init segment to recover the `AudioSpecificConfig`). With the feature: a `decode` module wires symphonia's AAC decoder + 16 kHz mono nearest-neighbour resampler against the recovered ASC, and a `worker` module owns a dedicated OS thread (not a tokio task -- whisper.cpp inference is CPU-bound) that buffers PCM up to `WhisperConfig::window_ms` (default 5 s), runs `WhisperContext::full` with English-only `Greedy { best_of: 1 }` sampling, and emits `TranscribedCaption` values onto the public `CaptionStream`. Communication is `std::sync::mpsc::sync_channel(64)`; a full channel logs `warn!` and drops the frame so a slow worker never back-pressures the per-broadcast drain task. Worker shutdown drains the remaining PCM, runs one final inference pass, then exits. The `whisper` feature also gates `tests/whisper_basic.rs`, an `#[ignore]` integration test that requires `WHISPER_MODEL_PATH` to point at a `ggml-*.bin` file (instructions in the test docblock; ggml-tiny.en is ~75 MB and is intentionally NOT bundled in `lvqr-conformance/fixtures`). No CLI wiring (session 100 D); no HLS subtitle rendition wiring (session 99 C). | `cargo fmt --all` clean; `cargo clippy --workspace --all-targets --benches -- -D warnings` clean; `cargo test -p lvqr-agent-whisper` 27 lib tests passed (no-feature build); `cargo check -p lvqr-agent-whisper --features whisper` clean (gated build); `cargo test --workspace` 823 passed / 0 failed / 1 ignored (up from 796; +27 new lib tests). | **DONE (session 98)** |
| 99 | C | Captions track publish + HLS subtitle rendition wiring. Three-crate stitch: (1) `lvqr-hls` gains a `subtitles.rs` module with `SubtitlesServer` (sliding-window cue store + WebVTT segment renderer + captions playlist renderer with PROGRAM-DATE-TIME alignment), `MultiHlsServer::ensure_subtitles` / `subtitles` accessors mirroring the audio rendition pattern, `EXT-X-MEDIA TYPE=SUBTITLES,GROUP-ID="subs",NAME="English",DEFAULT=YES,AUTOSELECT=YES,LANGUAGE="en",URI="captions/playlist.m3u8"` on the master playlist when subtitles exist, and a `subtitles_group: Option<String>` on `VariantStream` so the primary variant declares `SUBTITLES="subs"`. New axum routes: `GET /hls/{broadcast}/captions/playlist.m3u8` and `GET /hls/{broadcast}/captions/seg-{msn}.vtt` (one cue per segment for v1; future work could batch). (2) `lvqr-agent-whisper` factory gains a `with_caption_registry(FragmentBroadcasterRegistry)` builder + the `CAPTIONS_TRACK_ID = "captions"` constant; the agent lazily creates a `(broadcast, "captions")` broadcaster on `on_start` (whisper-feature gated; `FragmentMeta::new("wvtt", 1000)` -- ms timescale) and the worker dual-publishes each cue to both the in-process `CaptionStream` AND the captions broadcaster (Fragment payload = UTF-8 cue text; `dts/duration` = wall-clock UNIX ms). `on_stop` calls `registry.remove(broadcast, "captions")` so the HLS captions drain task sees `Closed` and exits cleanly. (3) `lvqr-cli` ships `BroadcasterCaptionsBridge` mirroring `BroadcasterHlsBridge::install`: `on_entry_created` callback that early-returns for any track other than `"captions"`, subscribes synchronously, spawns one drain task per broadcast that reads `Fragment` values and pushes `CaptionCue` into `MultiHlsServer::ensure_subtitles`. Wired in `lvqr_cli::start` next to the LL-HLS bridge. `ServerHandle::fragment_registry()` + `TestServer::fragment_registry()` accessors added so integration tests can publish captions directly to the registry without wiring whisper.cpp. The new `crates/lvqr-cli/tests/captions_hls_e2e.rs` exercises the full flow: synthetic video fragment + synthetic caption fragment onto the registry, HTTP GETs the master/captions/.vtt playlists, asserts `EXT-X-MEDIA SUBTITLES` + `SUBTITLES="subs"` + .vtt body carries the cue text. The negative test asserts master.m3u8 omits SUBTITLES when no captions producer is wired. | `cargo fmt --all` clean; `cargo clippy --workspace --all-targets --benches -- -D warnings` clean; `cargo test -p lvqr-cli --test captions_hls_e2e` 2 passed; `cargo test -p lvqr-hls --lib` 50 passed (15 new subtitles + 1 new master playlist); `cargo test -p lvqr-agent-whisper` 28 passed (one new for `on_stop` without caption_registry); `cargo test --workspace` 843 passed / 0 failed / 1 ignored (up from 823; +20 across the new modules + e2e). | **DONE (session 99)** |
| 100 | D | `--whisper-model` CLI + lifecycle + E2E demo. `lvqr-cli` gains a default-OFF `whisper` Cargo feature wiring `dep:lvqr-agent` + `dep:lvqr-agent-whisper` + `lvqr-agent-whisper/whisper` + `lvqr-test-utils/whisper`, surfaced through the `full` meta-feature. `ServeConfig` gains a `#[cfg(feature = "whisper")] pub whisper_model: Option<PathBuf>` field mirroring the existing `c2pa: Option<C2paConfig>` shape. `lvqr_cli::start` calls `WhisperCaptionsFactory::new(WhisperConfig::new(path)).with_caption_registry(shared_registry.clone())` and installs it on a throwaway `AgentRunner::new().with_factory(factory).install(&shared_registry)` immediately after `BroadcasterCaptionsBridge::install`; the returned `AgentRunnerHandle` is held on `ServerHandle.agent_runner` (also `#[cfg(feature = "whisper")]`) so the per-broadcast drain tasks survive the server lifetime. `main.rs` gains a `#[cfg(feature = "whisper")] #[arg(long, env = "LVQR_WHISPER_MODEL")] whisper_model: Option<PathBuf>` flag with `--help` text documenting the v1 English-only + no-history limitations. `lvqr-test-utils` gains a parallel `whisper` feature + `with_whisper_model` builder + `agent_runner()` accessor so integration tests can exercise the factory install path end-to-end. | `cargo fmt --all` clean; `cargo clippy --workspace --all-targets --benches -- -D warnings` clean; `cargo clippy -p lvqr-cli -p lvqr-test-utils -p lvqr-agent-whisper --features whisper --all-targets -- -D warnings` clean (plus pre-existing clippy drive-by fixes in lvqr-agent-whisper's `worker::run` too-many-args and `whisper_basic.rs` unused-param); `cargo build --release -p lvqr-cli` compiled without pulling whisper-rs or symphonia into the default dep tree; `cargo test --workspace` 843 passed / 0 failed / 1 ignored (parity with session 99; the new inline tests and e2e test are whisper-feature-gated and thus absent from the default workspace run); `cargo test -p lvqr-cli --features whisper --lib` 2 new inline tests pass; `cargo test -p lvqr-cli --features whisper --test whisper_cli_e2e` 1 `#[ignore]`-ed integration test (exercised by setting `WHISPER_MODEL_PATH`). Manual demo recipe baked into the close-doc commit. | **DONE (session 100)** |
...
## 4.4 -- Cross-cluster federation (2 weeks, 3 sessions, 101-103; COMPLETE)
...
| 102 | B | Two-cluster integration test + per-track re-publish implementation. `federation.rs` gains `forward_broadcast` + `forward_track` helpers that subscribe the LVQR convention tracks (`0.mp4` video, `1.mp4` audio, `catalog`) on the remote `BroadcastConsumer`, create parallel `TrackProducer`s on a local `BroadcastProducer`, and pump groups + frames through a cancel-respecting copy loop. Per-track tasks spawn inside `run_link`'s matched-announcement arm so unrelated broadcasts never allocate forwarder state. `FederationLink` gains `disable_tls_verify: bool` (serde default false; secure) + `with_disable_tls_verify(bool)` builder so integration tests can accept the TestServer's self-signed relay cert without a trust-store dance. The CLI composition root (`lvqr_cli::start`) gets a `federation_links: Vec<FederationLink>` field on `ServeConfig` (feature-gated on `cluster`), builds a `FederationRunner` against the relay's `OriginProducer` when the list is non-empty, and holds the handle on `ServerHandle.federation_runner` (also feature-gated) for the server lifetime alongside a new `origin()` accessor for integration tests. `TestServerConfig` mirrors with `with_federation_link(..)` + `TestServer::origin()` + `TestServer::federation_runner()`. New `crates/lvqr-cli/tests/federation_two_cluster.rs` (~120 LOC) boots two TestServers, injects a broadcast on A via `origin().create_broadcast` / `append_group` / `write_frame`, connects a raw `moq_native::Client` to B's relay, reads the announcement stream, subscribes to `0.mp4`, and asserts the exact frame bytes A wrote. | `cargo fmt --all` clean; `cargo clippy --workspace --all-targets --benches -- -D warnings` clean; `cargo test -p lvqr-cluster --test federation_unit` 9 passed; `cargo test -p lvqr-cluster --lib` 42 passed (+2 inline for the TLS knob); `cargo test -p lvqr-cli --test federation_two_cluster` 1 passed; `cargo test --workspace` 861 passed / 0 failed / 1 ignored (up from 858; +3). | **DONE (session 102)** |
| 103 | C | Admin route `GET /api/v1/cluster/federation` + exponential-backoff reconnect on link failure. `lvqr-cluster::federation` gains `FederationConnectState` (`connecting` / `connected` / `failed`, lowercase serde), `FederationLinkStatus` (per-link snapshot: remote_url, forwarded_broadcasts, state, last_connected_at_ms, last_error, connect_attempts, forwarded_broadcasts_seen), and `FederationStatusHandle` (cloneable `Arc<RwLock<Vec<FederationLinkStatus>>>` with `snapshot()` + internal mutators). `FederationRunner::start` wires every per-link task through an outer retry loop: `run_link_once` performs a single connect + announcement-drain pass (now also selecting on `session.closed()` so remote peer shutdown surfaces as a recoverable error instead of pinning the loop in Connecting); the wrapper records Connecting / Connected / Failed transitions, increments `connect_attempts`, sleeps `next_delay(attempt)` (base 1 s, doubling to 60 s cap, ±10% symmetric jitter) with a cancel-arm, and loops until shutdown. New `CONNECT_TIMEOUT` (10 s) bounds a single attempt so a silently-dropped QUIC Initial does not hold the link in Connecting forever. `FederationRunner::status_handle()` exposes a cloneable read handle for the admin layer. `lvqr-admin` adds `AdminState::with_federation_status(handle)` + `federation_status()` accessor (feature-gated on `cluster`) and a new `GET /api/v1/cluster/federation` route returning `{"links": [FederationLinkStatus..]}`; when no handle is wired (single-node build, empty `federation_links`), the route answers 200 + `{"links":[]}` rather than 503 so poll-unconditionally tooling works. `lvqr-cli::start` threads `federation_runner.status_handle()` into `AdminState` alongside the existing cluster handle. `TestServerConfig::with_relay_addr(addr)` lets the reconnect integration test pre-reserve a relay port (the same-port rebind did not land for reasons covered below; the builder still lives for future uses). CLAUDE.md's plan-vs-code rule: reality differed from the one-liner in two places. First, `session.closed()` monitoring was not in the original scope but turned out necessary for Failed-state detection; second, the same-port reconnect integration test was dropped because B's active federation client keeps A's ex-UDP-port locked on macOS through quinn's Endpoint teardown, so the integration test proves the observability contract (`Connected -> Failed` transition, `connect_attempts` keeps growing during retry) rather than the same-port rebind. Reconnect semantics stay covered by `federation_unit` tests. | `cargo fmt --all` clean; `cargo clippy --workspace --all-targets --benches -- -D warnings` clean; `cargo test -p lvqr-cluster --lib` 51 passed (+9 new inline: `next_delay` jitter window at attempt 0, doubling, clamped at attempt 6/10/20; `FederationLinkStatus` JSON round-trip + lowercase serde; `FederationStatusHandle` init snapshot, mutators, clones share state, out-of-bounds noop); `cargo test -p lvqr-cluster --test federation_unit` 11 passed (+2 new: `runner_status_handle_reports_failed_after_initial_connect_error`, `status_handle_clones_observe_updates`); `cargo test -p lvqr-admin` 17 passed (+2 new: `federation_route_returns_empty_list_when_not_wired`, `federation_route_reports_configured_link_status`); `cargo test -p lvqr-cli --test federation_two_cluster` 1 passed (unchanged, no regression); `cargo test -p lvqr-cli --test federation_reconnect` 1 passed (new); `cargo test --workspace` 875 passed / 0 failed / 1 ignored (up from 861; +14). | **DONE (session 103)** |
...
## 4.6 -- Server-side transcoding (2 weeks, 3 sessions, 104-106; **COMPLETE**)
...
| 105 | B | Real GStreamer software-encoder pipeline behind a default-OFF `transcode` Cargo feature. New optional deps: `gstreamer 0.23` + `gstreamer-app` + `gstreamer-video` + `glib`. `SoftwareTranscoderFactory` + `SoftwareTranscoder` in `crates/lvqr-transcode/src/software.rs` implement the existing `TranscoderFactory` / `Transcoder` trait shape; pipeline `appsrc(video/quicktime) ! qtdemux ! h264parse ! avdec_h264 ! videoscale ! video/x-raw,width=W,height=H ! videoconvert ! x264enc bitrate=K threads=2 tune=zerolatency speed-preset=superfast key-int-max=60 ! h264parse ! mp4mux streamable=true fragment-duration=2000 ! appsink emit-signals=true sync=false` drives the default 720p / 480p / 240p ABR ladder into the shared `FragmentBroadcasterRegistry` as `<source>/<rendition>` broadcasts. Worker-thread pattern lifted from `lvqr-agent-whisper`: bounded `std::sync::mpsc::sync_channel(64)` feeds an OS thread that owns the pipeline; full-channel drops bump `lvqr_transcode_dropped_fragments_total`. Output side: appsink `new-sample` callback caches `BufferFlags::HEADER` buffers as `FragmentBroadcaster::init_segment` and emits each subsequent buffer as a `Fragment` under the `"0.mp4"` output track. New metrics `lvqr_transcode_output_fragments_total{transcoder, rendition}` + `lvqr_transcode_output_bytes_total{transcoder, rendition}`. Recursion guard (`\d+p` suffix heuristic on `ctx.broadcast`) prevents the registry callback from re-firing on output broadcasts. `lvqr-cli` gains a sibling `transcode` feature folded into `full`; composition-root wiring is explicit 106 C scope. Scope-adjusted from the plan's one-liner per CLAUDE.md's plan-vs-code rule: video-only for 105 B (AAC passthrough deferred to 106 C so it ships alongside the HLS master playlist composition that consumes it). 6 new feature-gated inline tests + 1 feature-gated integration test against the `cmaf-h264-baseline-360p-1s.mp4` conformance fixture. | `cargo fmt --all --check` clean; `cargo clippy --workspace --all-targets --benches -- -D warnings` clean; `cargo clippy -p lvqr-transcode --features transcode --all-targets -- -D warnings` clean; `cargo test -p lvqr-transcode` 16 passed + 1 doctest (feature-off path unchanged from 104 A); `cargo test -p lvqr-transcode --features transcode --lib` 22 passed (+6 new inline on software.rs); `cargo test -p lvqr-transcode --features transcode --test software_ladder` 1 passed (wall clock ~0.3 s after first build; x264enc output within +/-10% of target bitrate across three consecutive runs); `cargo test --workspace` 892 / 0 / 1 (default features, unchanged; new tests are `#[cfg(feature = "transcode")]`-gated and do not alter the default gate). Host requirement: GStreamer 1.22+ runtime + plugin set (base, good, bad, ugly, libav). | **DONE (session 105)** |
| 106 | C | `lvqr-cli` composition-root wiring + `--transcode-rendition <NAME>` repeatable CLI flag + `LVQR_TRANSCODE_RENDITION` env fallback (comma-separated) + `--source-bandwidth-kbps` operator override; feature-gated `ServeConfig::transcode_renditions: Vec<RenditionSpec>` + `source_bandwidth_kbps: Option<u32>`; `ServerHandle::transcode_runner()` accessor on the test harness. `start()` builds one `SoftwareTranscoderFactory` + one `AudioPassthroughTranscoderFactory` per rendition against the shared `FragmentBroadcasterRegistry`; the audio factory is always-available (zero GStreamer dep) and copies `<source>/1.mp4` fragments to `<source>/<rendition>/1.mp4` verbatim so each rendition is a self-contained mp4. LL-HLS master-playlist composition extended in `lvqr-hls`: `MultiHlsServer::set_ladder(Vec<RenditionMeta>)` + `set_source_bandwidth_bps(Option<u64>)` + `variant_siblings` helper feed `handle_master_playlist` so `/hls/<source>/master.m3u8` emits one `#EXT-X-STREAM-INF` per sibling broadcast (highest-to-lowest `BANDWIDTH`, relative URIs `./<rendition>/playlist.m3u8`). `SoftwareTranscoderFactory::skip_source_suffixes(impl IntoIterator<...>)` builder covers operators running custom rendition names; `TranscodeRunner::drive()` now refreshes `sub.meta()` before `on_start` so a late `set_init_segment` (the RTMP-bridge ordering) is picked up by the transcoder snapshot. `lvqr-test-utils` gains a matching `transcode` feature with `TestServerConfig::with_transcode_ladder(Vec<RenditionSpec>)` builder. **Scope-adjusted from the plan's one-liner per CLAUDE.md's plan-vs-code rule**: the plan promised "Hardware encoder feature flags; benchmark NVENC vs x264" but the session-106 briefing (and session 105 B handoff) split hardware encoders off as post-4.6 follow-ups so 4.6 closes feature-complete on software + composition + egress. Hardware encoders (NVENC, VideoToolbox, VAAPI, QSV) remain open work but do not block 4.6 COMPLETE. Integration test `crates/lvqr-cli/tests/transcode_ladder_e2e.rs` (feature-gated on `transcode` + `rtmp`, soft-skip when GStreamer plugins are absent) publishes via `rml_rtmp` against `TestServer::with_transcode_ladder(default_ladder())`, fetches the master + per-rendition audio playlists, and checks the `TranscodeRunner` counters go non-zero for each rendition's software + audio-passthrough factory. | `cargo fmt --all --check` clean; `cargo clippy --workspace --all-targets --benches -- -D warnings` clean; `cargo clippy -p lvqr-transcode --features transcode --all-targets -- -D warnings` clean; `cargo clippy -p lvqr-cli --features transcode --all-targets -- -D warnings` clean; `cargo test -p lvqr-transcode` 16 passed + 1 doctest (feature-off path parity); `cargo test -p lvqr-transcode --features transcode --lib` 31 passed (+8 over 105 B: 5 new audio passthrough + 2 new software `skip_source_suffixes` + 1 on the updated recursion guard); `cargo test -p lvqr-transcode --features transcode --test software_ladder` 1 passed (unchanged from 105 B); `cargo test -p lvqr-cli --features transcode` 39 passed; `cargo test -p lvqr-cli --features transcode --test transcode_ladder_e2e` 1 passed; `cargo test --workspace` 900 / 0 / 1 (up from 892; +8 on the default gate from the always-available audio-passthrough + HLS master-playlist inline tests). | **DONE (session 106)** |
...
## 4.7 -- Latency SLO scheduling (1 week, 2 sessions, 107-108; **COMPLETE**)
...
| 108 | B | Prometheus / Grafana alert pack + operator runbook + asset-hygiene test. `deploy/grafana/alerts/lvqr-slo.rules.yaml` ships five Prometheus rules tuned for LL-HLS's default 2 s target (critical p99 > 4 s, warning p99 > 2 s, warning p95 > 1.5 s, info p50 > 500 ms, warning no-recent-samples). `deploy/grafana/dashboards/lvqr-slo.json` is a Grafana schema-38 dashboard with four panels (p99 + p95 + p50 timeseries + per-`(broadcast, transport)` sample rate) that imports via Grafana UI or the provisioning YAML. `deploy/grafana/README.md` documents both import paths + threshold tuning. `docs/slo.md` is the operator runbook covering metric shape, `/api/v1/slo` response, per-alert troubleshooting checklists (each named section matches the runbook_url anchor in the rule pack), and the per-transport threshold decision table (HLS / DASH / WHEP / MoQ / WS). `docs/observability.md` gains a "Latency SLO (Tier 4 item 4.7)" section linking the rule pack + dashboard + runbook. New test file `crates/lvqr-admin/tests/slo_assets.rs` reads each asset off disk and asserts (a) the rule pack names every expected alert + references the canonical metric name + embeds the runbook URL from at least five alert annotations, (b) the Grafana dashboard JSON parses + carries the stable `uid="lvqr-slo"` + at least four panels + at least one panel queries the histogram bucket metric, (c) the operator runbook ships every alert-named anchor the rule pack links to. Deliberately string-based checks to avoid a `serde_yaml` dep for asset hygiene; the authoritative YAML validation runs in CI / operator tooling via `promtool check rules`. After this session Tier 4 is COMPLETE. | `cargo fmt --all --check` clean; `cargo clippy --workspace --all-targets --benches -- -D warnings` clean; `cargo test -p lvqr-admin --test slo_assets` 3 passed; `cargo test --workspace` 912 / 0 / 1 (up from 909; +3 on the default gate from the three asset-hygiene tests). | **DONE (session 108)** |

...
## 4.3 -- C2PA signed media (4 sessions, 91-94)
...
| 94 | B3 | Drain-task integration + admin verify route + E2E. Three moving pieces: (a) Broadcast-end lifecycle hook on `lvqr_fragment::FragmentBroadcasterRegistry`. Current surface has `on_entry_created`; add a matching `on_entry_removed` or a more general `LifecycleObserver` trait. Load-bearing primitive that 4.4 (cross-cluster federation) + 4.5 (AI agents) will also consume -- design the API shape before coding. Specifically decide: callback fires on Drop (risky, deadlock potential) vs. explicit `registry.remove()` (safer but caller must remove). (b) Persist init bytes to disk at first-segment-write time. Today `FragmentBroadcaster::meta()` holds them in memory only. Decision: flat `<archive>/<broadcast>/<track>/init.mp4` vs. `metadata.json` sidecar. Pick + document. (c) Extend `lvqr_cli::archive::BroadcasterArchiveIndexer::drain` to call `lvqr_archive::provenance::finalize_broadcast_signed` (or the `_with_signer` variant if HSM-backed in future) inside `tokio::task::spawn_blocking` when the drain task terminates AND `C2paConfig` is `Some`. (d) `GET /playback/verify/{broadcast}` admin route in `lvqr-cli` reading the signed asset + sidecar via `c2pa::Reader::from_manifest_data_and_stream`, returning `{ signer, valid, errors }` JSON. (e) E2E test at `crates/lvqr-cli/tests/c2pa_verify_e2e.rs`: RTMP publish, broadcaster disconnect, hit `/playback/verify`, assert manifest parses + creator matches. | `cargo test -p lvqr-cli --test c2pa_verify_e2e` green. | pending |

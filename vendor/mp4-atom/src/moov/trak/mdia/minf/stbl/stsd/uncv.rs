...
impl Atom for Cmpd {
    ...
    fn decode_body<B: Buf>(buf: &mut B) -> Result<Self> {
        ...
        let mut components: Vec<Component> = Vec::with_capacity((component_count as usize).min(4096));
    ...
    fn decode_body_ext<B: Buf>(buf: &mut B, ext: UncCExt) -> Result<Self> {
        match ext.version {
            ...
            UncCVersion::V0 => {
                ...
                let component_count = u32::decode(buf)?;
                let mut components = Vec::with_capacity((component_count as usize).min(4096));
                for _ in 0..component_count {

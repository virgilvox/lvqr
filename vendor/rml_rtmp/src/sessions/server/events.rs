...
pub enum ServerSessionEvent {
    ...
    /// The client has sent an AMF0 data message that the session does not have a typed
    /// handler for. Common use cases include `onCuePoint` payloads (Adobe RTMP convention
    /// for in-band metadata such as SCTE-35 ad markers wrapped as `scte35-bin64`). The
    /// `data` vector contains the AMF0 values as they arrived on the wire, including the
    /// leading method-name string. `app_name` and `stream_key` come from the publish
    /// session that is currently active on `stream_id`; both are empty when the data
    /// arrives outside an active publish.
    ///
    /// LVQR (session 152) added this variant to surface SCTE-35 onCuePoint scte35-bin64
    /// passthrough data that the upstream rml_rtmp crate silently dropped.
    Amf0DataReceived {
        app_name: String,
        stream_key: String,
        data: Vec<Amf0Value>,

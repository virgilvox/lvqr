...
impl ServerSession {
    ...
    fn handle_amf0_data(
        ...
        match first_element {
            ...
            other => {
                // LVQR session 152 patch: surface non-@setDataFrame AMF0 data
                // (notably `onCuePoint` carriages used for SCTE-35 ad-marker
                // passthrough) as a typed event so consumers can route the
                // payload. Re-prepend the method name byte we removed above so
                // the consumer sees the wire-shape vector intact.
                let app_name = self.connected_app_name.clone().unwrap_or_default();
                let stream_key = self
                    .active_streams
                    .get(&stream_id)
                    .and_then(|s| match s.current_state {
